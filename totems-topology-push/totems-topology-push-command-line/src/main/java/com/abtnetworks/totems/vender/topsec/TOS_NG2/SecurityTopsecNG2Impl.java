package com.abtnetworks.totems.vender.topsec.TOS_NG2;
 ...
 * @Description:
 */
public class SecurityTopsecNG2Impl extends OverAllGeneratorAbstractBean {
    ...
    public String generateAbsoluteTimeCommandLine(String name, String attachStr, AbsoluteTimeParamDTO absoluteTimeParamDTO,
        ...
        timeCommandLineBuffer.append(String.format(" schedule add name %s ", timeNameCommandLine));
    ...
    public String generatePeriodicTimeCommandLine(String name, String attachStr, PeriodicTimeParamDTO periodicTimeParamDTO,
        ...
        StringBuffer timeCommandLineBuffer = new StringBuffer();
        timeCommandLineBuffer.append(String.format(" schedule add name %s cyctype", timeNameCommandLine));
    ...
    public String deleteAbsoluteTimeCommandLine(String timeFlag,Map<String, Object> map, String[] args){
        StringBuffer timeCommandLineBuffer = new StringBuffer(" schedule delete name "+timeFlag);
        return timeCommandLineBuffer.toString();
    }
    ...
    public String generateSingleIpV4CommandLine(StatusTypeEnum statusTypeEnum, String[] singleIpArray, Map<String, Object> map, String[] args) throws Exception {
        ...
        StringBuffer ipv4ArrayCommandLine = new StringBuffer(" host");
    ...
    public String generateRangeIpV4CommandLine(StatusTypeEnum statusTypeEnum, IpAddressRangeDTO[] rangIpArray, Map<String, Object> map, String[] args) throws Exception {
        ...
        for (IpAddressRangeDTO ipAddressRangeDTO : rangIpArray) {
            ...
            if (statusTypeEnum.getStatus().equals(StatusTypeEnum.ADD.getStatus())) {
                rangeIpCl.append(" range add name " + addressName + " ip1 " + ipAddressRangeDTO.getStart() + " ip2 " + ipAddressRangeDTO.getEnd());
            } else if (statusTypeEnum.getStatus().equals(StatusTypeEnum.MODIFY.getStatus())) {
                rangeIpCl.append(" range modify name " + addressName + " ip1 " + ipAddressRangeDTO.getStart() + " ip2 " + ipAddressRangeDTO.getEnd());
            }
        }
    ...
    public String generateSubnetIntIpV4CommandLine(StatusTypeEnum statusTypeEnum, IpAddressSubnetIntDTO[] subnetIpArray, String sub, Map<String, Object> map, String[] args) throws Exception {
        ...
        if (ArrayUtils.isNotEmpty(subnetIpArray)) {
            for (IpAddressSubnetIntDTO ipAddressSubnetIntDTO : subnetIpArray) {
                ...
                if (statusTypeEnum.getStatus().equals(StatusTypeEnum.ADD.getStatus())) {
                    subnetIpv4Cl.append(" subnet add name " + addressName + " ipaddr " + ipAddressSubnetIntDTO.getIp() + " mask " + ipAddressSubnetIntDTO.getMask());
                } else if (statusTypeEnum.getStatus().equals(StatusTypeEnum.MODIFY.getStatus())) {
                    subnetIpv4Cl.append(" subnet modify name " + addressName + " ipaddr " + ipAddressSubnetIntDTO.getIp() + " mask " + ipAddressSubnetIntDTO.getMask());
                }
            }
        }
    ...
    public String generateSubnetStrIpV4CommandLine(StatusTypeEnum statusTypeEnum, IpAddressSubnetStrDTO[] subnetIpArray, String sub, Map<String, Object> map, String[] args) throws Exception {
        ...
        StringBuffer subnetIpv4Cl = new StringBuffer();
        if (ArrayUtils.isNotEmpty(subnetIpArray)) {
            for (IpAddressSubnetStrDTO ipAddressSubnetIntDTO : subnetIpArray) {
                if (MapUtils.isEmpty(map)) {
                    addressName = this.createIpAddressObjectNameByIpMask(ipAddressSubnetIntDTO.getIp(),  Integer.valueOf( ipAddressSubnetIntDTO.getMask() ) , map, args);
                } else {
                    addressName = map.get("name").toString();
                }
                if (statusTypeEnum.getStatus().equals(StatusTypeEnum.ADD.getStatus())) {
    ...
    public String deleteIpAddressObjectCommandLine(RuleIPTypeEnum ipTypeEnum, String delStr, String name, Map<String, Object> map, String[] args) throws Exception {
        ...
        if(StringUtils.isNotEmpty(delStr)){
            switch (delStr) {
                case "host":
                    stringBuilder.append("define host delete name ").append(name);
                    break;
                case "range":
                    stringBuilder.append("define range delete name ").append(name);
                    break;
                case "subnet":
                    stringBuilder.append("define subnet delete name ").append(name);
                    break;
            }
        }
    ...
    public String generateIpAddressObjectGroupCommandLine(StatusTypeEnum statusTypeEnum, RuleIPTypeEnum ipTypeEnum,
        ...
        ipAddressGroupCl.append(" group_address add name " + name + " member '");
        // 本次生成的地址对象
    ...
    public String generateTCPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer tcpCommandLine = new StringBuffer(" service");
        ...
        if (StringUtils.isNotBlank(dstPortBuffer.toString())) {
            if(dstPortBuffer.toString().contains(" ")){
                ...
                // 单端口
                tcpCommandLine.append(String.format(" ports %s",dstPortBuffer.toString()));
            }
        }
    ...
    public String generateUDPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer udpCommandLine = new StringBuffer(" service");
        if (ObjectUtils.isNotEmpty(statusTypeEnum) && statusTypeEnum.getStatus().equalsIgnoreCase(StatusTypeEnum.DELETE.getStatus())) {
    ...
    public String generateICMPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer icmpCommandLine = new StringBuffer(" service");
        if (ObjectUtils.isNotEmpty(statusTypeEnum) && statusTypeEnum.getStatus().equalsIgnoreCase(StatusTypeEnum.DELETE.getStatus())) {
    ...
    public String generateOtherCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer otherCommandLine = new StringBuffer(" service");
        if (ObjectUtils.isNotEmpty(statusTypeEnum) && statusTypeEnum.getStatus().equalsIgnoreCase(StatusTypeEnum.DELETE.getStatus())) {
    ...
    public String generateServiceObjectGroupCommandLine(StatusTypeEnum statusTypeEnum,
        ...
        serviceObjectGroupCl.append(" group_service add name ");
    ...
    public String generateSecurityPolicyCommandLine(StatusTypeEnum statusTypeEnum, String groupName, String name, String id, String action, String description,
        ...
        if (ObjectUtils.isNotEmpty(srcIpDto)) {
            ...
            if (ArrayUtils.isNotEmpty(srcIpDto.getSingleIpArray())) {
                if(srcIpDto.getSingleIpArray().length <= 1){
                    ...
                    //临时获取地址对象组的名称 后期改
                    String singleIpObjectName ="add_"+String.valueOf(RandomUtils.nextInt());
            ...
            if (ArrayUtils.isNotEmpty(srcIpDto.getSubnetIntIpArray())) {
                ...
                if(subnetIntIpArray.length <= 1){
                    for (IpAddressSubnetIntDTO ipAddressSubnetIntDTO : subnetIntIpArray) {
                        newSrcIpAddressObjectNameList.add(this.createIpAddressObjectNameByIpMask(ipAddressSubnetIntDTO.getIp(), ipAddressSubnetIntDTO.getMask(), map, args));
                    }
        ...
        if (ObjectUtils.isNotEmpty(dstIpDto)) {
            ...
            if (ArrayUtils.isNotEmpty(dstIpDto.getSubnetIntIpArray())) {
                if(dstIpDto.getSubnetIntIpArray().length <= 1){
                    String subnetIntIpObjectName = this.createIpAddressObjectNameByIpSubArray(dstIpDto.getSubnetIntIpArray(), null, map, args);
        ...
        if ( null!= securityPolicyCl &&  StringUtils.isNotBlank( securityPolicyCl.toString() )  ){
            securityPolicyCl =  new StringBuffer( "define"+ StringUtils.LF + securityPolicyCl.append("end").toString() );
        }
        ...
        securityPolicyCl.append("firewall").append(StringUtils.LF).append("policy add action " + action);
        ...
        if(StringUtils.isNotBlank(groupName)){
            securityPolicyCl.append(" group_name "+groupName);
        }

        if(StringUtils.isNotBlank(description)){
            securityPolicyCl.append(" comment "+description);
        }
        ...
        if(moveSeatEnum != null){
            if(MoveSeatEnum.BEFORE.getCode() == moveSeatEnum.getCode()){
                securityPolicyCl.append(String.format(" before %s ", swapRuleNameId));
            }
        }
        securityPolicyCl.append( StringUtils.LF).append("end").append( StringUtils.LF);
    ...
    public String createIpAddressObjectNameByIpMask(String ip, int netmask, Map<String, Object> map, String[] args) {
        return StringUtils.join("SUB_",ip, SymbolsEnum.UNDERLINE.getValue(),netmask);
    }
    ...
    public String createIpAddressObjectNameByIpSubArray(IpAddressSubnetIntDTO[] subnetIntIpArray, IpAddressSubnetStrDTO[] subnetStrIpArray, Map<String, Object> map, String[] args) {
        ...
        if(ArrayUtils.getLength(subnetIntIpArray) + ArrayUtils.getLength(subnetStrIpArray) == 1){
            if(ArrayUtils.isNotEmpty(subnetIntIpArray) && StringUtils.isNotBlank(subnetIntIpArray[0].getIp())){
                return String.format("SUB_%s_%s",subnetIntIpArray[0].getIp(),subnetIntIpArray[0].getMask());
            }
            if(ArrayUtils.isNotEmpty(subnetStrIpArray)){
                return String.format("SUB_%s_%s",subnetStrIpArray[0].getIp(),TotemsIpUtils.getMaskBit(subnetStrIpArray[0].getMask()));
            }
        }
        int hashNum = 0;
