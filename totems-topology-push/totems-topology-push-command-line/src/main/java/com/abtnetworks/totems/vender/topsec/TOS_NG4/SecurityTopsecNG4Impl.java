package com.abtnetworks.totems.vender.topsec.TOS_NG4;
 ...
 * @Description:
 */
public class SecurityTopsecNG4Impl extends OverAllGeneratorAbstractBean {
    ...
    public String deleteIpAddressObjectCommandLine(RuleIPTypeEnum ipTypeEnum, String delStr, String name, Map<String, Object> map, String[] args) throws Exception {
        ...
        if(null != ipTypeEnum && "2".equals(ipTypeEnum.getCode())){
            ...
            switch (delStr) {
                case "host":
                    stringBuilder.append("define ipv6_host delete name " + name);
                    break;
                case "range":
                    stringBuilder.append("define ipv6_range delete name " + name);
                    break;
                case "subnet":
                    stringBuilder.append("define ipv6_subnet delete name " + name);
                    break;
                default:
                    stringBuilder.append("define ipv6_host delete name " + name);
                    break;
            ...
            switch (delStr) {
                case "host":
                    stringBuilder.append("define host delete name " + name);
                    break;
                case "range":
                    stringBuilder.append("define range delete name " + name);
                    break;
                case "subnet":
                    stringBuilder.append("define subnet delete name " + name);
                    break;
                default:
                    stringBuilder.append("define host delete name " + name);
                    break;
    ...
    public String generateTCPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        if (StringUtils.isNotBlank(srcPortBuffer.toString())) {
            tcpCommandLine.append(" ports " + srcPortBuffer.toString());
        }
        if (StringUtils.isNotBlank(dstPortBuffer.toString())) {
            if(dstPortBuffer.toString().contains(" ")){
                ...
                tcpCommandLine.append(String.format(" port1 %s port2 %s",dstPortBuffers[0],dstPortBuffers[1]));
            }else{
                // 单端口
                tcpCommandLine.append(String.format(" port1 %s",dstPortBuffer.toString()));
            }
        }
    ...
    public String generateUDPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        if (StringUtils.isNotBlank(srcPortBuffer.toString())) {
            udpCommandLine.append(" ports " + srcPortBuffer.toString());
        }
        if (StringUtils.isNotBlank(dstPortBuffer.toString())) {
            if(dstPortBuffer.toString().contains(" ")){
                // 范围端口
                String[] dstPortBuffers =  dstPortBuffer.toString().split(" ");
                udpCommandLine.append(String.format(" port1 %s port2 %s",dstPortBuffers[0],dstPortBuffers[1]));
            }else{
                // 单端口
                udpCommandLine.append(String.format(" port1 %s",dstPortBuffer.toString()));
            }
        }
    ...
    public String generateICMPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        if (StringUtils.isNotBlank(dstPortBuffer.toString())) {
            if(dstPortBuffer.toString().contains(" ")){
                // 范围端口
                String[] dstPortBuffers =  dstPortBuffer.toString().split(" ");
                icmpCommandLine.append(String.format(" port1 %s port2 %s",dstPortBuffers[0],dstPortBuffers[1]));
            }else{
                // 单端口
                icmpCommandLine.append(String.format(" port1 %s",dstPortBuffer.toString()));
            }
        }
