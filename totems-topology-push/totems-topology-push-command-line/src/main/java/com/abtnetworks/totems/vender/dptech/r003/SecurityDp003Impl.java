package com.abtnetworks.totems.vender.dptech.r003;
 ...
 * @Description:
 */
public class SecurityDp003Impl extends OverAllGeneratorAbstractBean {
    ...
    public String generateAbsoluteTimeCommandLine(String name, String attachStr, AbsoluteTimeParamDTO absoluteTimeParamDTO,
        ...
        StringBuffer timeCommandLineBuffer = new StringBuffer("time-range " + name + StringUtils.LF);
    ...
    public String generateSingleIpV4CommandLine(StatusTypeEnum statusTypeEnum, String[] singleIpArray, Map<String, Object> map, String[] args) throws Exception {
        ...
        } else if (statusTypeEnum.getStatus().equals(StatusTypeEnum.DELETE.getStatus())) {
            deleteFlag = "no ";
        }
        if (MapUtils.isEmpty(map)) {
            addressName = this.createIpAddressObjectNameBySingleIpArray(singleIpArray, map, args);
        } else {
        ...
        ipv4ArrayCommandLine.append(String.format("ip-obj-mask %s \n" , addressName));
        for (String ip : singleIpArray) {
            ipv4ArrayCommandLine.append(" ip-mask ").append(ip).append("/32 \n");
        }
        ipv4ArrayCommandLine.append("exit \n");
    ...
    public String generateRangeIpV4CommandLine(StatusTypeEnum statusTypeEnum, IpAddressRangeDTO[] rangIpArray, Map<String, Object> map, String[] args) throws Exception {
        ...
        for (IpAddressRangeDTO ipAddressRangeDTO : rangIpArray) {
            ...
            rangeIpCl.append(String.format("ip-obj-range %s \n",addressName));
            rangeIpCl.append(deleteFlag).append(" ip-range start-ip " + ipAddressRangeDTO.getStart() + " end-ip " + ipAddressRangeDTO.getEnd()).append("\n");
            rangeIpCl.append("exit \n");
        }
    ...
    public String generateSubnetIntIpV4CommandLine(StatusTypeEnum statusTypeEnum, IpAddressSubnetIntDTO[] subnetIpArray, String sub, Map<String, Object> map, String[] args) throws Exception {
        ...
        if (ArrayUtils.isNotEmpty(subnetIpArray)) {
            for (IpAddressSubnetIntDTO ipAddressSubnetIntDTO : subnetIpArray) {
                if (MapUtils.isEmpty(map)) {
                    addressName = this.createIpAddressObjectNameByIpMask(ipAddressSubnetIntDTO.getIp(), ipAddressSubnetIntDTO.getMask(), map, args);
                } else {
                    addressName = map.get("name").toString();
                }
                subnetIpv4Cl.append("ip-obj-mask " + addressName).append("\n");
                if (statusTypeEnum.getStatus().equals(StatusTypeEnum.MODIFY.getStatus())) {
                    ...
                    subnetIpv4Cl.append(" ip-mask " + ipAddressSubnetIntDTO.getIp() + "/" + ipAddressSubnetIntDTO.getMask());
                }
                subnetIpv4Cl.append(StringUtils.LF);
                subnetIpv4Cl.append("exit");
            }
        }
        ...
        if (ArrayUtils.isNotEmpty(subnetIpArray)) {
            for (IpAddressSubnetStrDTO ipAddressSubnetStrDTO : subnetIpArray) {
                ...
                if (statusTypeEnum.getStatus().equals(StatusTypeEnum.MODIFY.getStatus())) {
                    ...
                    subnetIpv4Cl.append(" ip-mask " + ipAddressSubnetStrDTO.getIp() + "/" + TotemsIpUtils.getMaskBit(ipAddressSubnetStrDTO.getMask()));
                }
                subnetIpv4Cl.append(StringUtils.LF);
                subnetIpv4Cl.append("exit");
            }
    ...
    public String generateSingleIpV6CommandLine(StatusTypeEnum statusTypeEnum, String[] singleIpArray, Map<String, Object> map, String[] args) throws Exception {
        ...
        if(ArrayUtils.isNotEmpty(singleIpArray)){
            StringBuffer singIpCl = new StringBuffer();
            for (String singleIp : singleIpArray) {
                singIpCl.append(String.format("ipv6 address-object %s %s/128 \n",addressName,singleIp));
            }
            return singIpCl.toString();
        }
    ...
    public String generateRangeIpV6CommandLine(StatusTypeEnum statusTypeEnum, IpAddressRangeDTO[] rangIpArray, Map<String, Object> map, String[] args) throws Exception {
        String addressName = null;
        if (MapUtils.isEmpty(map)) {
            addressName = this.createIpAddressObjectNameByRangIpArray(rangIpArray, map, args);
        } else {
            addressName = map.get("name").toString();
        }
        if(ArrayUtils.isNotEmpty(rangIpArray)){
            ...
            for (IpAddressRangeDTO rangeIp : rangIpArray) {
                rangeIpCl.append(String.format("ipv6 address-object %s %s %s \n",addressName,rangeIp.getStart(),rangeIp.getEnd()));
            }
            return rangeIpCl.toString();
        }
    ...
    public String generateSubnetIntIpV6CommandLine(StatusTypeEnum statusTypeEnum, IpAddressSubnetIntDTO[] subnetIpArray, String sub, Map<String, Object> map, String[] args) throws Exception {
        String addressName = null;
        if (MapUtils.isEmpty(map)) {
            addressName = this.createIpAddressObjectNameByIpSubArray(subnetIpArray,null, map, args);
        } else {
            addressName = map.get("name").toString();
        }
        if(ArrayUtils.isNotEmpty(subnetIpArray)){
            StringBuffer subnetIntCl = new StringBuffer();
            for (IpAddressSubnetIntDTO subnetIntDTO : subnetIpArray) {
                subnetIntCl.append(String.format("ipv6 address-object %s %s/%s \n",addressName,subnetIntDTO.getIp(),subnetIntDTO.getMask()));
            }
            return subnetIntCl.toString();
        }
    ...
    public String deleteIpAddressObjectCommandLine(RuleIPTypeEnum ipTypeEnum, String delStr, String name, Map<String, Object> map, String[] args) throws Exception {
        StringBuilder stringBuilder = new StringBuilder("no ip-obj " + name);
    ...
    public String generateIpAddressObjectCommandLine(StatusTypeEnum statusTypeEnum, RuleIPTypeEnum ipTypeEnum,
        ...
        //单ip
        if (ArrayUtils.isNotEmpty(singleIpArray) && getArraysLength(singleIpArray,rangIpArray,subnetIntIpArray,subnetStrIpArray) <= 1)  {
        ...
        } else if (ArrayUtils.isEmpty(singleIpArray) && (getArraysLength(singleIpArray,rangIpArray,subnetIntIpArray,subnetStrIpArray) >= 2)) {
            //生成地址组对象
            commandLine.append(this.generateIpAddressObjectGroupCommandLine(statusTypeEnum, ipTypeEnum, name, idStr, singleIpArray, rangIpArray,
                    subnetIntIpArray, subnetStrIpArray, interfaceArray, fqdnArray, objectNameRefArray, null, description, attachStr, delStr, map, args));
        } else {
    ...
    public String generateTCPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer tcpCommandLine = new StringBuffer(deleteFlag+"usr-service " + map.get("name").toString() + " tcp");
    ...
    public String generateUDPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer udpCommandLine = new StringBuffer(deleteFlag+"usr-service " + map.get("name").toString() + " udp");
        if (StringUtils.isNotBlank(srcPortBuffer.toString())) {
    ...
    public String generateICMPCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuilder icmpCommandLine = new StringBuilder(deleteFlag+"usr-service " + map.get("name").toString() + " icmp ");
    ...
    public String generateOtherCommandLine(StatusTypeEnum statusTypeEnum, String[] protocolAttachTypeArray, String[] protocolAttachCodeArray,
        ...
        StringBuffer otherCommandLine = new StringBuffer(deleteFlag+"usr-service " + map.get("name").toString() + " protocol ");
    ...
    public String generateServiceObjectGroupCommandLine(StatusTypeEnum statusTypeEnum,
        ...
        serviceObjectGroupCl.append(StringUtils.LF);
        serviceObjectGroupCl.append("group-service " + name).append("\n");
        if (CollectionUtils.isNotEmpty(newServiceObjectNameList)) {
            for (String serviceObjectName : newServiceObjectNameList) {
                serviceObjectGroupCl.append(" usr-service " + serviceObjectName+StringUtils.LF);
            }
        }
        ...
        if (ArrayUtils.isNotEmpty(serviceObjectGroupNameRefArray)) {
            for (String serviceObjectGroupName : serviceObjectGroupNameRefArray) {
                serviceObjectGroupCl.append("usr-service " + serviceObjectGroupName+StringUtils.LF);
            }
        }
        ...
        if (ObjectUtils.isNotEmpty(srcIpDto)) {
            ...
            if (ArrayUtils.isNotEmpty(srcIpDto.getRangIpArray())) {
                if(srcIpDto.getRangIpArray().length > 1){
                    ...
                    newSrcIpAddressGroupNameList.add(singleIpObjectName);
                    ipAddressGroupCl.append(this.generateIpAddressObjectGroupCommandLine(statusTypeEnum, dstIpDto.getIpTypeEnum(), singleIpObjectName, null, null, srcIpDto.getRangIpArray(), null, null, null, null,
                            null, null,null, null, null, map, args));
                }else{
        ...
        if (ObjectUtils.isNotEmpty(dstIpDto)) {
            ...
            if (ArrayUtils.isNotEmpty(dstIpDto.getRangIpArray())) {
                if(dstIpDto.getRangIpArray().length <= 1){
                    ...
                    ipAddressGroupCl.append(this.generateIpAddressObjectGroupCommandLine(statusTypeEnum, dstIpDto.getIpTypeEnum(), rangeIpObjectName, null, null, dstIpDto.getRangIpArray(), null, null, null, null,
                            null,null, null, null, null, map, args));
                }
            ...
            if (ArrayUtils.isNotEmpty(dstIpDto.getSubnetIntIpArray())) {
                if(dstIpDto.getSubnetIntIpArray().length <= 1){
                    ...
                    ipAddressGroupCl.append(this.generateIpAddressObjectGroupCommandLine(statusTypeEnum, dstIpDto.getIpTypeEnum(), subnetIntIpObjectName, null, null, null, dstIpDto.getSubnetIntIpArray(), null, null,
                            null, null, null,null, null, null, map, args));
                }
            }
            if (ArrayUtils.isNotEmpty(dstIpDto.getSubnetStrIpArray())) {
                if(dstIpDto.getSubnetStrIpArray().length <= 1){
                    ...
                    ipAddressGroupCl.append(this.generateIpAddressObjectGroupCommandLine(statusTypeEnum, dstIpDto.getIpTypeEnum(), subnetStrIpObjectName, null, null, null, null, dstIpDto.getSubnetStrIpArray(), null,
                            null, null, null,null, null, null, map, args));
                }
            }
        }
    ...
    public int getArraysLength( String[] singleIpArray, IpAddressRangeDTO[] rangIpArray,
        ...
        if(ArrayUtils.isNotEmpty(subnetStrIpArray)){
            strSubnet = subnetStrIpArray.length;
        }
        return single+range+subnet+strSubnet;
    }
}
