...
/**
 * @Author: WangCan
 * @Description CiscoASA 9.9命令行
 * @Date: 2021/6/10
 */
public class SecurityCiscoASA99Impl extends OverAllGeneratorAbstractBean {
    ...
    public String generateSecurityPolicyCommandLine(StatusTypeEnum statusTypeEnum, String groupName, String name, String id, String action, String description,
        ...
        if (map != null && map.containsKey("isCiscoInterfaceCreate") && (boolean)map.get("isCiscoInterfaceCreate")) {
            if (map.containsKey("outBound") && (boolean)map.get("outBound")) {
                securityPolicyCl.append(String.format("access-group %s out interface %s ", name, dstZoneName));
            } else {
                securityPolicyCl.append(String.format("access-group %s in interface %s ", name, srcZoneName));
            }
        }
        if (  StringUtils.isNotBlank( description ) && StringUtils.isNotBlank( swapRuleNameId ) ){
            securityPolicyCl.append( String.format("access-list %s line %s remark  %s", name,  (Integer.parseInt(swapRuleNameId)+1) , description ) );
        }
        securityPolicyCl.append(StringUtils.LF);
    ...
    public String deleteAbsoluteTimeCommandLine(String name, Map<String, Object> map, String[] args) {
        if (StringUtils.isNotEmpty(name)) {
            return String.format("no time-range %s %s", name, StringUtils.LF);
        } else {
            return null;
        }
    }
