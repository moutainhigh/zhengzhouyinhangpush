package com.abtnetworks.totems.checkpoint;
...
public class checkpointCommandLineTest {

    @Test
    public void testCPCommandLine() throws Exception {
        PolicyParamDTO paramDTO = new PolicyParamDTO();
        paramDTO.setAction("permit");
        paramDTO.setDescription("checkpoint命令行生成");
...
//        paramDTO.setAbsoluteTimeParamDTO(absoluteTimeParamDTO);
        ...
        SecurityCheckpointImpl cp = new SecurityCheckpointImpl();
        System.out.println("cp命令行------------------》");
        Map map = new HashMap<>();
        map.put("objectName","test");
        System.out.println(cp.generateIpAddressObjectCommandLine(StatusTypeEnum.ADD,RuleIPTypeEnum.IP6,"test","15",new String[]{"1.1.1.1","5.5.5.5"},
