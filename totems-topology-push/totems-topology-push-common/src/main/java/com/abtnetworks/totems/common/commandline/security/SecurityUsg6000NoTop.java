...
/**
 * @desc    华为usg6000 no top命令行
 * @author liuchanghao
 * @date 2020-12-18 15:55
 */
@Service
public class SecurityUsg6000NoTop extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityUsg6000NoTop.class);
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, List<String> existServiceNameList,Integer ipType) {
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            createServiceObjectNames.add(name);
            if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
        ...
        if(CollectionUtils.isNotEmpty(existServiceNameList)){
            for(String existServiceName : existServiceNameList ){
                existServiceName = containsQuotes(existServiceName);
                joinSb.append(String.format("service %s \n", existServiceName));
            }
        }
...
//        SecurityUsg6000NoTop usg6000Notop = new SecurityUsg6000NoTop();
