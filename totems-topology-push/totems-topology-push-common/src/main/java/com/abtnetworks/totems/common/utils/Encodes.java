...
//	}
	...
	public static String decodeBase64Key(String input) {
		try {
			if(input != null && !"".equals(input)) {
				String result = new String(Base64.decodeBase64(input.getBytes()), DEFAULT_URL_ENCODING);
				return result.substring(KEY1.length(), result.length() - KEY2.length());
			}
			return "";
		} catch (UnsupportedEncodingException e) {
			return "";
	...
	public static void main(String args[]) {
		...
		String pass = Encodes.decodeBase64Key(password);
