...
@Service
public class RollbackSnatLongMa implements PolicyGenerator {
    @Override
    ...
    /**
     * @return no ip nat destination 策略id号
     */
    @Override
    public PolicyGeneratorDTO generateV2(CmdDTO cmdDTO) {
        ...
        if(!StringUtils.isEmpty(policyId)){
            sb.append("enable\nconfigure terminal\n");
            sb.append("no ip nat source ").append(policyId).append(StringUtils.LF);
            sb.append("end\nsave config\n");
        }
