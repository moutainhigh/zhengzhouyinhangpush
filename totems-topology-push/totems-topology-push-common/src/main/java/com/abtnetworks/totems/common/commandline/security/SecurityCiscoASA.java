...
@Slf4j
@Service
public class SecurityCiscoASA extends SecurityPolicyGenerator implements PolicyGenerator {
    ...
    public String generateCommandline(CommandlineDTO dto) {
        ...
        return commonLine(srcAddress, dstAddress, serviceObject, dto,description);
    }
    ...
    public String commonLine(PolicyObjectDTO srcAddress, PolicyObjectDTO dstAddress, PolicyObjectDTO serviceObject,
        ...
        /**
         *  2022.03.03市场问题 思科的长连接设置是对了这个策略集下面的所有的策略，这个不对，所以解决方案是 只要是带长链接的工单 都去新建一个策略集 然后对这个策略集再设置长连接信息
         access-list sk204  extended permit object-group tcp_231  object sk204_AO_8797 object sk204_AO_1879
         class-map sk204_T_8737
         match access-list sk204
         exit
         policy-map sk204_AO_5402
         class sk204_T_8737
         set connection timeout idle 100:0:0
         * **/
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, boolean createObjFlag, String existsAddressName,String IpSystem) {
        ...
        List<String>  createAddressGroupObejctNames = new ArrayList<>();
        String[] arr = ipAddress.split(",");
        List<String> list = new ArrayList<>();
        StringBuilder sb = new StringBuilder();
        // 是创建对象
        if (createObjFlag) {
            String objName = StringUtils.isEmpty(IpSystem) ? String.format("%s_AO_%s", ticket, IdGen.getRandomNumberString()) : IpSystem;
            dto.setName(objName);
            ...
            dto.setObjectFlag(true);
            createAddressGroupObejctNames.add(objName);
        } else {
            //直接显示内容
            formatFullAddress(arr, list, sb);
        ...
        dto.setCreateGroupObjectName(createAddressGroupObejctNames);
        return dto;
    }
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, String existsServiceName) {
        ...
        if (StringUtils.isNotBlank(existsServiceName)) {
            dto.setObjectFlag(true);
            if(PolicyConstants.POLICY_STR_VALUE_ICMP.equalsIgnoreCase(existsServiceName)) {
                dto.setJoin(String.format(" %s ", existsServiceName.toUpperCase()));
            }else{
        ...
        if(PolicyConstants.POLICY_STR_VALUE_ICMP.equalsIgnoreCase(name)){
            dto.setName(name.toUpperCase());
            dto.setJoin(String.format(" %s ", name.toUpperCase()));
        }else {
            dto.setName(name);
    ...
    public static void main(String[] args) {
        SecurityCiscoASA ciscoASA = new SecurityCiscoASA();
