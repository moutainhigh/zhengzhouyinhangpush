...
@Slf4j
@Service
public class RoutingDPTechFirewall implements RoutingGenerator , PolicyGenerator {
     ...
     * ipv6 route 1111:2222::aaaa/128 null0
     * @param dto
     * @return
        ...
        if (dto.getRoutingType().equals(RoutingCommandDTO.RoutingType.UNREACHABLE)) {
            ...
            allIpList.forEach(ip -> {
                if (ip.contains(":")) {
                    ...
                    //ipv4
                    stringBuilder.append("ip route ").append(ip).append("/32 null0\n");
                }
            });
        }
    ...
    /**
     * no ip route 1.1.1.1/32 null0
     * no ipv6 route 1111:2222::aaaa/128 null0
        ...
        if (routingCommandDTO.getRoutingType().equals(RoutingCommandDTO.RoutingType.UNREACHABLE)) {
            ...
            allIpList.forEach(ip -> {
                if (ip.contains(":")) {
                    ...
                    //ipv4
                    stringBuilder.append("no ip route ").append(ip).append("/32 null0\n");
                }
            });
        }
