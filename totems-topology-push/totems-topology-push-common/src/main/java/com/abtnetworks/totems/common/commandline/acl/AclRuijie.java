...
@Service
@Log4j2
public class AclRuijie extends SecurityPolicyGenerator implements PolicyGenerator {

    @Override
    public String generate(CmdDTO cmdDTO) {
        log.info("开始锐捷acl 命令行生成");
        ...
        log.info("锐捷 ACL dto is" + JSONObject.toJSONString(dto, true));
    ...
    public String commonLine(PolicyObjectDTO srcAddress, PolicyObjectDTO dstAddress,
        ...
        // 当前可用的policyIds
        List<Integer> usableRuleList = dto.getUsableRuleList();
        ...
        for (ServiceDTO service : dto.getServiceList()) {
            ...
            if (dstPortList != null && dstPortList.size() > 0) {
                    ...
                    for (int j = 0; j < srcPortList.size(); j++) {
                        for (int m = 0; m < dstAddressList.size(); m++) {
                            for (int n = 0; n < dstPortList.size(); n++) {
                                if (CollectionUtils.isEmpty(usableRuleList)) {
                                    sb.append(String.format(" %s %s %s %s %s %s", dto.getAction().toLowerCase(), protocolString,
                                            srcAddressList.get(i), srcPortList.get(j), dstAddressList.get(m), dstPortList.get(n)));
                                } else {
                                    sb.append(String.format("%d %s %s %s %s %s %s",
                                            usableRuleList.get(usableIndex), dto.getAction().toLowerCase(), protocolString,
                                            srcAddressList.get(i), srcPortList.get(j), dstAddressList.get(m), dstPortList.get(n)));
                                }
                                if (time != null) {
                ...
                for (int i = 0; i < srcAddressList.size(); i++) {
                    for (int m = 0; m < dstAddressList.size(); m++) {
                        if (CollectionUtils.isEmpty(usableRuleList)) {
                            sb.append(String.format(" %s %s %s %s", dto.getAction().toLowerCase(), protocolString,
