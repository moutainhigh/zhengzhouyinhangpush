...
import com.abtnetworks.totems.vender.Legendsec.security.SecurityLegendsecImpl;
...
@Slf4j
@Service
public class SecurityLegendSecV40 extends SecurityPolicyGenerator implements PolicyGenerator {

    private SecurityLegendsecImpl generatorBean;

    public SecurityLegendSecV40() {
        generatorBean = new SecurityLegendsecImpl();
    }

    private static final Logger logger = LoggerFactory.getLogger(SecurityLegendSecV40.class);

    private static final String SECURITY_POLICY = "security policy ";
    private static final String SZONE = " szone ";
    private static final String DZONE = " szone ";
    private static final String GROUP_TEMPLATE = "%s_group_%s";
    private static final String OBJECT_SERVICE_GROUP = "object service-group ";
    private static final String SERVICE_TEMPLATE = "service %s\n";
    ...
    public String commonLine(PolicyObjectDTO srcAddress, PolicyObjectDTO dstAddress, PolicyObjectDTO serviceObject,
        ...
        if (time != null) {
            sb.append(SECURITY_POLICY).append(setName).append(" schedule ").append(time.getJoin()).append(CommonConstants.LINE_BREAK);
        }
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, boolean createObjFlag, String existsAddressName, String ipPrefix) {
        ...
        StringBuilder sb = new StringBuilder();
        IpAddressParamDTO ipAddressParamDTO = Param4CommandLineUtils.buildIpAddressParamDTO(ipAddress);
        join = generatorBean.createIpAddressObjectNameByParamDTO(ipAddressParamDTO.getSingleIpArray(),ipAddressParamDTO.getRangIpArray(),ipAddressParamDTO.getSubnetIntIpArray(),ipAddressParamDTO.getSubnetStrIpArray(),ipAddressParamDTO.getHosts(),ipAddressParamDTO.getObjectNameRefArray(),null,null);
        try {
            if (isIpV6) {
                sb.append(generatorBean.generateIpAddressObjectCommandLine(StatusTypeEnum.ADD, RuleIPTypeEnum.IP6, join, null, ipAddressParamDTO.getSingleIpArray(),
                        ipAddressParamDTO.getRangIpArray(), ipAddressParamDTO.getSubnetIntIpArray(), ipAddressParamDTO.getSubnetStrIpArray(), null, null,
                        null, null, null, null, null, null));

            } else {
                sb.append(generatorBean.generateIpAddressObjectCommandLine(StatusTypeEnum.ADD, RuleIPTypeEnum.IP4, join, null, ipAddressParamDTO.getSingleIpArray(),
                        ipAddressParamDTO.getRangIpArray(), ipAddressParamDTO.getSubnetIntIpArray(), ipAddressParamDTO.getSubnetStrIpArray(), null, null,
                        null, null, null, null, null, null));
            }
        }catch (Exception e){
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, boolean isIpv6, List<String> existsServiceNames) {
        ...
        if (CollectionUtils.isEmpty(serviceDTOList)) {
            if (CollectionUtils.isNotEmpty(existsServiceNames)) {
                if (existsServiceNames.size() > 1) {
                    ...
                    String serviceName = String.format(GROUP_TEMPLATE, nameGroup, IdGen.getRandomNumberString());
                    sb.append(OBJECT_SERVICE_GROUP).append(serviceName).append(CommonConstants.LINE_BREAK);
                    for (String existServiceName : existsServiceNames) {
                        sb.append(String.format(SERVICE_TEMPLATE, existServiceName));
                    }
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP) && !isIpv6) {
                ...
                String dstPort = dstPorts[0];
                String srcPort = srcPorts[0];
                //是TCP/UPD协议
                //源为any，目的端口有值，则仅显示目的端口
                String portString = PortUtils.getPortString(dstPort, PortUtils.BLANK_FORMAT);
                if(StringUtils.isNotEmpty(portString) && !(portString.contains(" ") || portString.contains("-") || portString.contains("_") || portString.contains(":"))){
                    portString = String.format("%s %s",portString,portString);
                }
                if (srcPorts1.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY) && !dstPorts1.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY)) {
                    sb.append(String.format("service-item %s src-port 1 65535 dst-port %s\n", protocolString, portString));
                    ...
                    if (srcPort.equals(PolicyConstants.POLICY_STR_VALUE_ANY) && dstPort.equals(PolicyConstants.POLICY_STR_VALUE_ANY)) {
                        ...
                        sb.append(String.format("service-item %s src-port %s dst-port %s\n", protocolString, PortUtils.getPortString(srcPort, PortUtils.BLANK_FORMAT), portString));
                    }
                }
        ...
        if (sizeTotal > 1) {
            ...
            sb.append(EXIT);
            serviceName = String.format(GROUP_TEMPLATE, serviceName, IdGen.getRandomNumberString());
            sb.append(OBJECT_SERVICE_GROUP).append(serviceName).append(CommonConstants.LINE_BREAK);
            if (isCreate) {
                sb.append(String.format(SERVICE_TEMPLATE, name));
            }
            ...
            if(sizeTotal == 1 && isCreate ){
                ...
                sb.append(OBJECT_SERVICE_GROUP).append(serviceName).append(CommonConstants.LINE_BREAK);
                sb.append(String.format(SERVICE_TEMPLATE, name));
                sb.append(String.format(SERVICE_TEMPLATE, existsServiceNames.get(0)));
                join = String.format("%s", serviceName);
