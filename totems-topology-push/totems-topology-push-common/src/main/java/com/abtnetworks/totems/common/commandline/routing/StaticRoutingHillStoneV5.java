...
@Service
public class StaticRoutingHillStoneV5 implements PolicyGenerator {

    private StaticRoutingHillStoneV5Impl stoneV5;

    public StaticRoutingHillStoneV5() {
    ...
    public String generate(CmdDTO cmdDTO) {
        ...
        try {
            ...
            if (IpTypeEnum.IPV4.getCode() == policy.getIpType().intValue()){
                sb.append(stoneV5.generateIpv4RoutingCommandLine(policy.getDstIp(),routingInfoDTO.getSubnetMask(),
                        routingInfoDTO.getNextHop(),routingInfoDTO.getOutInterface(),routingInfoDTO.getPriority(),null, policy.getDescription(),map,null));
            }else {
                sb.append(stoneV5.generateIpv6RoutingCommandLine(policy.getDstIp(),routingInfoDTO.getSubnetMask(),
                        routingInfoDTO.getNextHop(),routingInfoDTO.getOutInterface(),routingInfoDTO.getPriority(),null, policy.getDescription(),map,null));
            }
