...
@Slf4j
@Service(value = "KFW NAT")
public class  KFWNat implements NatPolicyGenerator{

    @Override
    public String generate(CmdDTO cmdDTO) {
        log.info("generate KFW NAT 策略");
    ...
    public String generateStaticNatCommandLine(StaticNatTaskDTO policyDTO) {
        ...
        if(StringUtils.isNotEmpty(policyDTO.getProtocol()) && isExist==false){
            ...
            if (!protocol.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                if(StringUtils.isNotEmpty(policyDTO.getGlobalPort()) && !StringUtils.equalsAnyIgnoreCase(policyDTO.getGlobalPort(),PolicyConstants.POLICY_STR_VALUE_ANY)){
                    sb.append("define firewall service custom\n");
    ...
    public String generateDNatCommandLine(DNatPolicyDTO policyDTO) {
        ...
        dstIp=dstIps[0];
        if(dstIp.contains("-")){
            String[] dstIprange=dstIp.split("-");
            dstIp=dstIprange[0];
        }
        if(dstIp.contains("/")){
            int endindex=dstIp.lastIndexOf(".");
            String dstIps2=dstIp.substring(0,endindex);
            dstIp=dstIps2+".0";
        }
        ...
        if((!dstPorts.equals("any")) || (!postPort.equals("any"))){
            ...
            if (!protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                if(StringUtils.isNotEmpty(dstPorts) && !StringUtils.equalsAnyIgnoreCase(dstPorts,PolicyConstants.POLICY_STR_VALUE_ANY)){
                    String[] dstPort=dstPorts.split(",");
                    dstPorts=dstPort[0];
                    if(dstPorts.contains("-")){
                        String[] dstIprange=dstPorts.split("-");
                        dstPorts=dstIprange[0];
        ...
        if(CollectionUtils.isNotEmpty(restSrcAddressList)){
            for(String srcAddress : restSrcAddressList){
                sb.append("define firewall address\n");
                String element = "";
            ...
            if (!protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                if (StringUtils.isNotEmpty(dstPorts) && !StringUtils.equalsAnyIgnoreCase(dstPorts, PolicyConstants.POLICY_STR_VALUE_ANY)) {
                    ...
                    if (PortUtils.isPortRange(dstPorts)) {
                        ...
                        serviceName = String.format("\"%s_%s-%s\"", protocolString.substring(0, 3).toLowerCase(), startPort, endPort);
                    } else {
                        serviceName = String.format("\"%s_%s\"", protocolString.substring(0, 3).toLowerCase(), dstPorts);
                    }
                } else {
                    serviceName = protocolString.toUpperCase();
                }
            } else {
                serviceName = "ICMP_ANY";
            }
        }
...
//        }
    ...
    private String createServiceObject(ServiceDTO service, List<String> serviceNameList) {
        ...
        if (service.getDstPorts().equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY)) {
            if(protocol.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)){
                serviceNameList.add(String.format("ICMP_ANY"));
            }else {
    ...
    public PolicyObjectDTO generateAddressPool(String ipAddress, String ticket){
        ...
        for (String address : arr) {
            sb.append("define firewall ippool\n");
        ...
        String name1= ArrayUtils.toString(name,",");
...
//
        ...
        dNatPolicyDTO.setDstIp("172.16.2.2,172.16.2.17/24");

        dNatPolicyDTO.setPostIpAddress("1.1.1.1,192.168.1.0/24");
        ...
        System.out.println(kfwNat.generateDNatCommandLine(dNatPolicyDTO));
