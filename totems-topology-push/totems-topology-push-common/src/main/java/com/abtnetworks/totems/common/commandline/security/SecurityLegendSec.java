...
@Slf4j
@Service
public class SecurityLegendSec extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = LoggerFactory.getLogger(SecurityLegendSec.class);
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, boolean createObjFlag, String existsAddressName, String ipPrefix) {
        ...
        if (isIpV6) {
            ...
            sb.append("object6 network address ").append(join).append(CommonConstants.LINE_BREAK);
            ...
            for (String address : arr) {
                ...
                if (address.contains("/")) {
                    ...
                    if (StringUtils.isNotEmpty(addrArray[0])) {
                        addressCmd = String.format("network-object6 range %s %s\n", addrArray[0].toLowerCase(), addrArray[1]);
                    }
                    ...
                    addressCmd = String.format("network-object6 host %s\n", address.toLowerCase());
                }
            ...
            sb.append("object network address ").append(join).append(CommonConstants.LINE_BREAK);

            for (String address : arr) {
                if (IpUtils.isIPRange(address)) {
                    String startIp = IpUtils.getStartIpFromRange(address);
                    String endIp = IpUtils.getEndIpFromRange(address);
                    addressCmd = String.format("network-object range  %s %s\n", startIp, endIp);
                    sb.append(addressCmd);
                    ...
                    addressCmd = String.format("network-object network %s  %s\n", ip, maskBit);
                    sb.append(addressCmd);
                } else {
                    sb.append("network-object host ").append(address).append(CommonConstants.LINE_BREAK);
                }
            }
