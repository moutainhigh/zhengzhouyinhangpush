 ...
 */
@Service
public class SecurityHillStoneR5 extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityHillStoneR5.class);
    ...
    public String editCommandLine(CommandlineDTO dto) {
        ...
        PolicyObjectDTO srcAddressObject = generateAddressObject(dto.getSrcIp(), ticket, "src", createObjFlag, dto.getSrcAddressName(), dto.getSrcIpSystem(), dto.getIpType(),createObjectName,dto.getUrlType());
        PolicyObjectDTO dstAddressObject = generateAddressObject(dto.getDstIp(), ticket, "dst", createObjFlag, dto.getDstAddressName(), dto.getDstIpSystem(), dto.getIpType(),createObjectName,dto.getUrlType());
    ...
    public String createCommandLine(CommandlineDTO dto) {
        ...
        if (StringUtils.isNotEmpty(existSrcName)) {
            srcAddress = generateAddressObject(srcIp, ticket, "src", createObjFlag, existSrcName, dto.getSrcIpSystem(), dto.getIpType(),null,dto.getUrlType());
        } else {
            srcAddress = generateAddressObject(dto.getExistSrcAddressList(), dto.getRestSrcAddressList(), ticket, "src", createObjFlag, dto.getSrcIpSystem(), dto.getIpType(),createObjectName,dto.getUrlType());
        }
        ...
        if (StringUtils.isNotEmpty(existDstName) && ObjectUtils.isNotEmpty(dto.getRestDstAddressList())) {
            dstAddress = generateAddressObject(dstIp, ticket, "dst", createObjFlag, existDstName, dto.getDstIpSystem(), dstIpType,null,dto.getUrlType());
        } else {
            dstAddress = generateAddressObject(dto.getExistDstAddressList(), dto.getRestDstAddressList(), ticket, "dst", createObjFlag, dto.getDstIpSystem(), dstIpType,createObjectName,dto.getUrlType());
        }
    ...
    public PolicyObjectDTO generateAddressObject(List<String> existAddressList, List<String> restAddressList, String ticket,
                                                 String ipPrefix, boolean createObjFlag, String ipSystem, Integer ipType,List<String> createObjectName,Integer urlType) {
        ...
        if(ipType.intValue() == IpTypeEnum.IPV4.getCode()){
            ...
            if(null == urlType || UrlTypeEnum.IPV4.getCode() == urlType.intValue()){
                policyObjectDTO.setJoin(ipPrefix + "-addr any\n");
            } else if(UrlTypeEnum.IPV6.getCode() == urlType.intValue()){
                policyObjectDTO.setJoin(ipPrefix + "-addr IPv6-any\n");
            }
        }
        ...
        if (restAddressList.size() > 0) {
            ...
            policyObjectDTO = generateAddressObject(sb.toString(), ticket, ipPrefix, createObjFlag, "", ipSystem, ipType,createObjectName,urlType);
        }
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName, String ipSystem, Integer ipType,List<String> createObjectName, Integer urlType) {
        PolicyObjectDTO dto = new PolicyObjectDTO();
        if(CollectionUtils.isEmpty(createObjectName)){
            createObjectName = new ArrayList<>();
        }
        ...
        // 获取对象名称
        String name = getObjectName(ticket, ipSystem);

        name = String.format("\"%s\"", name);

        if (IpTypeEnum.URL.getCode() == ipType.intValue()){
            if (null != urlType && UrlTypeEnum.IPV6.getCode() == urlType.intValue()){
        ...
        for (String address : arr) {
            // 是创建对象
            formatFullAddress(address, sb, ipPrefix, createObjFlag, dto, name, ipType);
        }
     ...
     * @return
     */
    private String getObjectName(String ticket, String ipSystem) {
        String name;
    ...
    public PolicyObjectDTO generateAddressObjectForNat(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName, String ipSystem,Integer ipType) {
        ...
        if (arr.length > 1) {
            for (String address : arr) {
                formatFullAddress(address, sb, ipPrefix, true, dto, name, ipType);
            }
            sb.append("exit\n");
            dto.setName(name);
            dto.setCommandLine(sb.toString());
            createObjectName.add(name);
        } else {
            if (arr.length == 1) {
                //直接显示内容
                formatFullAddress(arr[0], sb, ipPrefix, false, dto, name, ipType);
                dto.setCommandLine(sb.toString());
                dto.setName(sb.toString());
            }
    ...
    private void formatFullAddress(String address, StringBuilder sb, String ipPrefix, boolean createObjFlag, PolicyObjectDTO dto, String name, Integer ipType) {
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, Integer idleTimeout, String existsServiceName) {
        ...
        if (StringUtils.isNotBlank(existsServiceName) && idleTimeout == null) {
            dto.setObjectFlag(true);
        ...
        if (serviceDTOList != null && serviceDTOList.size() == 1 && idleTimeout == null) {
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            if (StringUtils.isNotBlank(command) && idleTimeout == null) {
                groupFlag = true;
            ...
            String serviceName = String.format("\"%s%s\"", protocolString.substring(0, 1).toUpperCase(), service.getDstPorts());
            if (idleTimeout != null) {
                serviceName = String.format("\"%s%sL\"", protocolString.substring(0, 1).toUpperCase(), service.getDstPorts());
            }
            if (PortUtils.isPortRange(service.getDstPorts())) {
                ...
                serviceName = String.format("\"%s%s-%s\"", protocolString.substring(0, 1).toUpperCase(), start, end);
                if (idleTimeout != null) {
                    serviceName = String.format("\"%s%s-%sL\"", protocolString.substring(0, 1).toUpperCase(), start, end);
                }
            }
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        List<String>  createServiceObject = new ArrayList<>();
        //多个，建对象
        for (ServiceDTO service : serviceDTOList) {
            int protocolNum = Integer.valueOf(service.getProtocol());
            String protocolString = ProtocolUtils.getProtocolByValue(protocolNum).toLowerCase();
        ...
        dto.setCreateServiceObjectName(createServiceObject);
        dto.setCommandLine(sb.toString());
        return dto;
    }
    ...
    public static void main(String[] args) {
        ...
        SecurityHillStoneR5 hillStoneR5 = new SecurityHillStoneR5();
        String commandLine = hillStoneR5.createCommandLine(dto);
