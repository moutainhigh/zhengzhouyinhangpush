...
@Slf4j
@Service()
public class LeadSecPowerNat implements NatPolicyGenerator {
    ...
    public String generateStaticNatCommandLine(StaticNatTaskDTO policyDTO) {
        ...
        SecurityLeadSecPowerV securityLeadSecPowerV = new SecurityLeadSecPowerV();
        PolicyObjectDTO insidePolicyObjectDTO = securityLeadSecPowerV.generateAddressObject(insideAddress, policyDTO.getTheme(), policyDTO.isCreateObject(), policyDTO.getInsideAddressName());
        PolicyObjectDTO globalAddressObjectDTO = securityLeadSecPowerV.generateAddressObject(globalAddress, policyDTO.getTheme(), false, policyDTO.getGlobalAddressName());
        //定义对象
    ...
    public String generateSNatCommandLine(SNatPolicyDTO policyDTO) {
        SecurityLeadSecPowerV securityLeadSecPowerV = new SecurityLeadSecPowerV();
        PolicyObjectDTO srcAddressObject = securityLeadSecPowerV.generateAddressObject(policyDTO.getSrcIp(), policyDTO.getTheme(), policyDTO.isCreateObjFlag(), policyDTO.getSrcAddressObjectName());
        PolicyObjectDTO dstAddressObject = securityLeadSecPowerV.generateAddressObject(policyDTO.getDstIp(), policyDTO.getTheme(), policyDTO.isCreateObjFlag(), policyDTO.getDstAddressObjectName());
...
// 转换后源地址对象定义（地址池，不要复用）,目的nat要复用：
    ...
    public static void main(String[] args) {
        LeadSecPowerNat r004 = new LeadSecPowerNat();
        System.out.println("--------------------------------------------------------------------------");
        //源nat
