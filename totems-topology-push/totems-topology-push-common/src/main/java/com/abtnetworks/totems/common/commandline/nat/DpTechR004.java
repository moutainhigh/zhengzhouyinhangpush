 ...
 */
@Slf4j
@Service(value = "DpTechR004 NAT")
public class DpTechR004 implements NatPolicyGenerator {
    ...
    public String generateSNatCommandLine(SNatPolicyDTO policyDTO) {
        ...
        PolicyObjectDTO srcAddressObject = securityDpTechR004.generateAddressObject(policyDTO.getSrcIp(), policyDTO.getTheme(), policyDTO.getSrcAddressObjectName(), policyDTO.getSrcIpSystem());
        PolicyObjectDTO dstAddressObject = securityDpTechR004.generateAddressObject(policyDTO.getDstIp(), policyDTO.getTheme(), policyDTO.getDstAddressObjectName(), policyDTO.getDstIpSystem());

        List<String> exitNameForService = new ArrayList<>();
        List<ServiceDTO> restForService = new ArrayList<>();
        // 统计部分复用的情况
        securityDpTechR004.covertServiceParam(policyDTO.getServiceObjectName(),policyDTO.getExistServiceNameList(),policyDTO.getRestServiceList() ,restForService ,exitNameForService);
        List<PolicyObjectDTO> serviceObjectList = securityDpTechR004.generateServiceObject(restForService);

        //源地址 转换后， 生成地址池对象  每次都新建
        PolicyObjectDTO srcPostObject = generateAddressPool(policyDTO.getPostIpAddress(), policyDTO.getTheme(), policyDTO.getPostAddressObjectName(),policyDTO.getPostSrcIpSystem());
        ...
        if (CollectionUtils.isNotEmpty(exitNameForService)) {
        ...
        //源地址转换后 地址池引用
        if (StringUtils.isNotBlank(srcPostObject.getJoin())) {
            //KSH-4801 添加action
    ...
    public static void main(String[] args) {
        DpTechR004 r004 = new DpTechR004();
        System.out.println("--------------------------------------------------------------------------");
        //源nat
...
//        sNatPolicyDTO.setSrcIp("192.168.2.1,192.168.2.2");
        ...
        sNatPolicyDTO.setSrcIp("172.16.2.4");
