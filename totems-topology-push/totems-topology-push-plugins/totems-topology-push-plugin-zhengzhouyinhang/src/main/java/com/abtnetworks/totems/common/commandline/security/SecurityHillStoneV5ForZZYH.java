...
@Slf4j
@Service
@CustomCli(value = DeviceModelNumberEnum.HILLSTONE_V5, type = PolicyEnum.SECURITY)
public class SecurityHillStoneV5ForZZYH extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityHillStoneV5ForZZYH.class);
    ...
    public SecurityHillStoneV5ForZZYH() {
        init();
    }
    ...
    public String editCommandLine(CommandlineDTO dto) {
        ...
        PolicyObjectDTO srcAddressObject = generateAddressObject(dto.getSrcIp(), ticket, "src", createObjFlag, dto.getSrcAddressName(), dto.getSrcIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
        PolicyObjectDTO dstAddressObject = generateAddressObject(dto.getDstIp(), ticket, "dst", createObjFlag, dto.getDstAddressName(), dto.getDstIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
...
//        }
        ...
        if(StringUtils.isNotEmpty(existSrcName)){
            srcAddress = generateAddressObject(srcIp,ticket,"src",createObjFlag,existSrcName,dto.getSrcIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
        }else {
            srcAddress = generateAddressObject(dto.getExistSrcAddressList(), dto.getRestSrcAddressList(), ticket, "src", createObjFlag, dto.getSrcIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
        }
        String existDstName = dto.getDstAddressName();
        PolicyObjectDTO dstAddress;
        if(StringUtils.isNotEmpty(existDstName)){
            dstAddress = generateAddressObject(dstIp,ticket,"dst",createObjFlag,existDstName,dto.getDstIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
        }else{
            dstAddress  = generateAddressObject(dto.getExistDstAddressList(), dto.getRestDstAddressList(), ticket, "dst", createObjFlag, dto.getDstIpSystem(), dto.getIpType(),dto.getUrlType(), dto.getDescription());
        }
        ...
        if (StringUtils.isNotEmpty(dto.getSrcIpSystem()) && StringUtils.isNotEmpty(dto.getDstIpSystem()) && StringUtils.isNotEmpty(dto.getDescription())) {
            ...
            sb.append(String.format("name %s\n", "\"" + dto.getName() + "_" + "AO"+ "_" + IdGen.getRandomNumberString() + "\""));
        }
...
//        }
        ...
        if(isDstSingle&&StringUtils.isNotEmpty(dto.getDstIp())){
            ...
            if(IpTypeEnum.IPV4.getCode() == dto.getIpType().intValue()){
                ...
                // 域名
                fullStr = String.format("dst-host %s\n", dto.getDstIp());
            }
            sb.append(fullStr);
    ...
    public PolicyObjectDTO generateAddressObject(List<String> existAddressList, List<String>restAddressList, String ticket,
                                                 String ipPrefix, boolean createObjFlag,String ipSystem, Integer ipType,Integer urlType, String description) {
        ...
        if(restAddressList.size() > 0) {
            ...
            policyObjectDTO = generateAddressObject(sb.toString(), ticket, ipPrefix, createObjFlag,"", ipSystem,ipType,urlType, description);
        }
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName, String ipSystem, Integer ipType,Integer urlType, String description) {
        ...
        if (ipType.intValue() == IpTypeEnum.URL.getCode() && urlType.intValue() == UrlTypeEnum.IPV6.getCode()){
            sb.append(String.format("address %s %s\n", name, "ipv6"));
            joinSB.append(ipPrefix + "-addr " + name + "\n");
        }else if (ipType.intValue() == IpTypeEnum.IPV6.getCode()) {
            sb.append(String.format("address %s %s\n", name, "ipv6"));
            joinSB.append(ipPrefix + "-addr " + name + "\n");
        }
        ...
        for (String address : arr) {
            // 是创建对象
            formatFullAddress(address, sb, ipPrefix, createObjFlag, dto, name, arr.length, ipType, createObjectNames, ipSystem, joinSB);
        }
    ...
    /**
     * 自定义生成address名称,只针对ipv4
     * @param ipAddress     IP地址
     * @param ipSystem      自定义对象名称
     * @return
     */
    private String designGenarateAddressName(String ipAddress, String ipSystem) {
        String name = "";
        if (IpUtils.isIP(ipAddress) || IpUtils.isIPSegment(ipAddress)) {
            name = ipAddress;
        } else if (IpUtils.isIPRange(ipAddress)) {
            //地址为范围的时候，如果第三位的数字不一样了，就要变成全称
    ...
    private void formatFullAddress(String address, StringBuilder sb, String ipPrefix, boolean createObjFlag, PolicyObjectDTO dto, String name,int length, Integer ipType, List<String>  createObjectNames, String ipSystem, StringBuffer joinSB) {
        String fullStr = "";
        if(IpTypeEnum.IPV4.getCode()== ipType.intValue()){
            name = designGenarateAddressName(address, ipSystem);
            sb.append(String.format("address %s\n", "\"" + name + "\""));
            joinSB.append(ipPrefix + "-addr " + name + "\n");
            if (IpUtils.isIPSegment(address)) {
                fullStr = String.format("ip %s\n", address);
            ...
            // 域名
            if("src".equalsIgnoreCase(ipPrefix)) {
                name = designGenarateAddressName(address, ipSystem);
                if (IpUtils.isIPSegment(address)) {
                    sb.append(String.format("address %s\n", "\"" + name + "\""));
                    joinSB.append(ipPrefix + "-addr " + name + "\n");
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        for(ServiceDTO service : serviceDTOList){
            ...
            if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_TCP)
                ...
                for (String dstPort : dstPorts) {
                    if(idleTimeout != null) {
                        names = String.format("\"L-%s_%s\"", protocolString.substring(0, 3).toLowerCase(), dstPort);
                    }
                    if(PortUtils.isPortRange(dstPort)) {
                        String start = PortUtils.getStartPort(dstPort);
                        String end = PortUtils.getEndPort(dstPort);
                        names =  String.format("\"%s_%s-%s\"", protocolString.substring(0, 3).toLowerCase(), start, end);
                        if(idleTimeout != null) {
                            names =  String.format("\"L-%s_%s-%s\"", protocolString.substring(0, 3).toLowerCase(), start, end);
                        }
                    }
                        ...
                        if(idleTimeout != null) {
                            ...
                            names=String.format("\"%s_%s\"", protocolString.substring(0, 3).toLowerCase(), dstPort);
                        }
    ...
    public static void main(String[] args) {
        ...
        dto.setVsys(true);
        dto.setVsysName("zy");
        SecurityHillStoneV5ForZZYH hillStoneR5 = new SecurityHillStoneV5ForZZYH();
