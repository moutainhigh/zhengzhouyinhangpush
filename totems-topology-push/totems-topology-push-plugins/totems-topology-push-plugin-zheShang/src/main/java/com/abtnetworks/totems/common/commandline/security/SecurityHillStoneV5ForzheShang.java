...
public class SecurityHillStoneV5ForzheShang extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityHillStoneV5ForzheShang.class);
    ...
    public SecurityHillStoneV5ForzheShang() {
        init();
    }
    ...
    public String editCommandLine(CommandlineDTO dto) {
        ...
        sb.append(StringUtils.LF).append("save");
...
//        }
        ...
        if(isSrcSingle&&StringUtils.isNotEmpty(dto.getSrcIp())) {
            for(String itemIp : split){
                String fullStr = "";
                if (IpUtils.isIPSegment(itemIp)) {
                    fullStr = String.format("src-ip %s\n", itemIp);
                } else if (IpUtils.isIPRange(itemIp)) {
                    String startIp = IpUtils.getStartIpFromRange(itemIp);
                    String endIp = IpUtils.getEndIpFromRange(itemIp);
                ...
                if(dto.getIpType().intValue() == IpTypeEnum.IPV6.getCode()){
                    ...
                    if (itemIp.contains("/")) {
                        fullStr = String.format("src-ip %s\n", itemIp);
                    } else if(itemIp.contains("-")){
                        // 范围
                        String startIp = IpUtils.getRangeStartIPv6(itemIp);
                        String endIp = IpUtils.getRangeEndIPv6(itemIp);
                        fullStr = String.format("src-range %s %s\n", startIp, endIp);
                    } else {
            ...
            {
                sb.append(String.format("src-addr any\n"));
            }
    ...
    public static void main(String[] args) {
        ...
        dto.setSrcIp("12.3.1.3,2.4.5.2,2.2.2.2");
        dto.setDstIp("4.2.3.2,3.4.2.4");
        dto.setIpType(0);
        dto.setRestDstAddressList(restDst);
        dto.setSrcIpSystem("cesojo");
        ...
        dto.setVsys(true);
        dto.setVsysName("zy");
        SecurityHillStoneV5ForzheShang hillStoneR5 = new SecurityHillStoneV5ForzheShang();
