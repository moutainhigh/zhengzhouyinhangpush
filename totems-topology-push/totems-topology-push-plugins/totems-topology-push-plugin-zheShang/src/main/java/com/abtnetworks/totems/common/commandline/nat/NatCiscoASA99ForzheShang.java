...
@Slf4j
@Service
public class NatCiscoASA99ForzheShang implements NatPolicyGenerator {
    ...
    public String generateStaticNatCommandLine(StaticNatTaskDTO policyDTO) {
        ...
        if(policyDTO.isRollback()){
            ...
            rollbacksb.append(String.format("no nat (%s,%s) source %s %s %s %s\n",
                    StringUtils.isBlank(policyDTO.getFromZone()) ? policyDTO.getInDevItf() : policyDTO.getFromZone(),
                    StringUtils.isBlank(policyDTO.getToZone()) ? policyDTO.getOutDevItf() : policyDTO.getToZone(),
                    "static",
    ...
    public String generateBothNatCommandLine(NatPolicyDTO policyDTO) {
        ...
        if(policyDTO.isRollback()){
            StringBuilder rollbacksb = new StringBuilder();
            rollbacksb.append("configure terminal\n");
            rollbacksb.append(String.format("no nat (%s,%s) source %s %s %s destination static %s %s %s\n", StringUtils.isBlank(policyDTO.getSrcZone()) ? policyDTO.getSrcItf() : policyDTO.getSrcZone(),
                    StringUtils.isBlank(policyDTO.getDstZone()) ? policyDTO.getDstItf() : policyDTO.getDstZone(),
    ...
    private PolicyObjectDTO getAddressObjectGroup(String ipAddresses, String name, String addressObjectName,String ipSystem, boolean createObject){
        ...
        // 如果页面上填了系统名称则根据系统名称来创建地址对象
        String objectName = null;
        if (StringUtils.isNotBlank(ipSystem)) {
            objectName = dealIpSystemName(ipSystem);
        } else {
            objectName = String.format("%s_AO_%s", name, IdGen.getRandomNumberString());
        }
        StringBuilder sb = new StringBuilder();
    ...
    public static void main(String[] args) {
        NatCiscoASA99ForzheShang cisco = new NatCiscoASA99ForzheShang();
...
//
        ...
        bothNatDTO.setSrcIp("");
        bothNatDTO.setDstIp("");
        bothNatDTO.setPostSrcIp("");
        bothNatDTO.setPostDstIp("");
