...
public class SecurityCiscoASAForJiaoYin extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = LoggerFactory.getLogger(SecurityCiscoASAForJiaoYin.class);
    ...
    public String commonLine(PolicyObjectDTO srcAddress, PolicyObjectDTO dstAddress, PolicyObjectDTO serviceObject,
        ...
        if (serviceObject.isObjectFlag() && srcAddress.isObjectFlag() && dstAddress.isObjectFlag()) {
            String[] servicees = serviceObject.getJoin().split(",");
            for (String servicee : servicees) {
                if (servicee.contains("tcp")) {
                    sb.append(String.format("access-list %s %s extended %s tcp %s %s %s", businessName, line1, dto.getAction().toLowerCase() , srcAddress.getJoin(), dstAddress.getJoin(), servicee));
                } else if (servicee.contains("udp")) {
                    sb.append(String.format("access-list %s %s extended %s udp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddress.getJoin(), dstAddress.getJoin(), servicee));
                } else if (servicee.contains("icmp")) {
                    sb.append(String.format("access-list %s %s extended %s icmp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddress.getJoin(), dstAddress.getJoin(), servicee));
                }
                if (time != null) {
        ...
        } else if (serviceObject.isObjectFlag() && StringUtils.isNotBlank(serviceObject.getJoin())
            ...
            for (String servicee : servicees) {
                for (int i = 0; i < srcAddressList.size(); i++) {
                    for (int j = 0; j < dstAddressList.size(); j++) {
                        if (servicee.contains("tcp")) {
                            sb.append(String.format("access-list %s %s extended %s tcp %s %s %s", businessName, line1, dto.getAction().toLowerCase() , srcAddressList.get(i), dstAddressList.get(j), servicee));
                        } else if (servicee.contains("udp")) {
                            sb.append(String.format("access-list %s %s extended %s udp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddressList.get(i), dstAddressList.get(j), servicee));
                        } else if (servicee.contains("icmp")) {
                            sb.append(String.format("access-list %s %s extended %s icmp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddressList.get(i), dstAddressList.get(j), servicee));
                        }
                        if (time != null) {
                            sb.append(time.getJoin());
                        }
            ...
            for (String servicee : servicees) {
                ...
                for (int j = 0; j < dstAddressList.size(); j++) {
                    if (servicee.contains("tcp")) {
                        sb.append(String.format("access-list %s %s extended %s tcp %s %s %s", businessName, line1, dto.getAction().toLowerCase() , srcAddress.getJoin(), dstAddressList.get(j), servicee));
                    } else if (servicee.contains("udp")) {
                        sb.append(String.format("access-list %s %s extended %s udp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddress.getJoin(), dstAddressList.get(j), servicee));
                    } else if (servicee.contains("icmp")) {
                        sb.append(String.format("access-list %s %s extended %s icmp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddress.getJoin(), dstAddressList.get(j), servicee));
                    }
                    if (time != null) {
                        sb.append(time.getJoin());
                    }
            ...
            for (String servicee : servicees) {
                ...
                for (int j = 0; j < srcAddressList.size(); j++) {
                    if (servicee.contains("tcp")) {
                        sb.append(String.format("access-list %s %s extended %s tcp %s %s %s", businessName, line1, dto.getAction().toLowerCase() , srcAddressList.get(j), dstAddress.getJoin(), servicee));
                    } else if (servicee.contains("udp")) {
                        sb.append(String.format("access-list %s %s extended %s udp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddressList.get(j), dstAddress.getJoin(), servicee));
                    } else if (servicee.contains("icmp")) {
                        sb.append(String.format("access-list %s %s extended %s icmp %s %s %s", businessName, line1, dto.getAction().toLowerCase(), srcAddressList.get(j), dstAddress.getJoin(), servicee));
                    }
                    if (time != null) {
                        sb.append(time.getJoin());
                    }
        ...
        } else if (srcAddress.isObjectFlag() && StringUtils.isNotBlank(srcAddress.getJoin())
                && dstAddress.isObjectFlag() && StringUtils.isNotBlank(dstAddress.getJoin())) {
        ...
        if (createObjFlag && StringUtils.isNotBlank(ipSystem)) {
            ...
            if (StringUtils.isNotBlank(ipSystem)) {
                ...
                for (String address : arr) {
                    objName2.append(String.format("host %s ",address));
                    sb.append(String.format("object-group network %s \n", address));
                    formatFullAddress(address, list, sb);
                }
                dto.setJoin(String.format("%s\n",objName2));
    ...
    public void formatFullAddress(String address, List<String> list, StringBuilder sb) {
        String fullStr = "";
        if (IpUtils.isIPSegment(address)) {
            //获取ip
            String ip = IpUtils.getIpFromIpSegment(address);
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, String existsServiceName) {
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            dto.setName(name);
            objName.append(String.format("object-group %s ,", name));
        ...
        dto.setJoin(String.format("%s", objName));
        sb.append("exit\n");
        dto.setCommandLine(sb.toString());
        dto.setObjectFlag(true);
        return dto;
    ...
    public static void main(String[] args) {
        SecurityCiscoASAForJiaoYin ciscoASA = new SecurityCiscoASAForJiaoYin();
