...
public class SecurityHillStoneR5ForhuaXia extends SecurityPolicyGenerator implements PolicyGenerator {
    ...
    public SecurityHillStoneR5ForhuaXia() {
        init();
    }
    ...
    public String createCommandLine(CommandlineDTO dto) {
        ...
        if(StringUtils.isNotEmpty(existSrcName)){
            ...
            srcAddress = generateAddressObject(dto.getExistSrcAddressList(), dto.getRestSrcAddressList(), ticket, "src", createObjFlag, dto.getDstIpSystem(), dto.getIpType());
        }
        ...
        if(dto.getIdleTimeout()!=null){
            name=String.format("%s_long", name);
        }
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName, String ipSystem, Integer ipType) {
        ...
        for (String address : arr) {
            // 是创建对象
            if (createObjFlag) {
                formatFullAddress(address, sb, ipPrefix, createObjFlag, dto, ticket,ipSystem, arr.length, ipType);
                sb.append("exit\n");
    ...
    public PolicyObjectDTO generateAddressObjectForNat(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName,String ipSystem) {
        ...
        if(arr.length>1){
            ...
            if(arr.length == 1){
                //直接显示内容
                formatFullAddress(arr[0], sb, ipPrefix, false, dto, ticket,ipSystem, arr.length, 0);
                dto.setCommandLine(sb.toString());
                dto.setName(sb.toString());
            }
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        for(ServiceDTO service : serviceDTOList){
            ...
            String name = String.format("\"%s%s\"", protocolString.substring(0, 3).toUpperCase(), service.getDstPorts());
            if(idleTimeout != null) {
                name = String.format("\"%s%s_long\"", protocolString.substring(0, 3).toUpperCase(), service.getDstPorts());
            }
            if(PortUtils.isPortRange(service.getDstPorts())) {
                ...
                name =  String.format("\"%s%s-%s\"", protocolString.substring(0, 3).toUpperCase(), start, end);
                if(idleTimeout != null) {
                    name =  String.format("\"%s%s-%s_long\"", protocolString.substring(0, 3).toUpperCase(), start, end);
            ...
            if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_TCP)
                ...
                for (String dstPort : dstPorts) {
                    ...
                    if(idleTimeout != null) {
                        ...
                        sb.append(String.format(" timeout-hour %d", idleTimeout/HOUR_SECOND));
                    }
                    sb.append("\n");
                }
            }else if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
...
//        restSrc.add("");
        ...
        restDst.add("1.3.2.56-12.3.2.4");
        restDst.add("1.3.88.55");
        dto.setIpType(0);
        dto.setIdleTimeout(3244657);
        ...
        serviceDTO.setDstPorts("543-89");
        serviceDTO2.setProtocol("17");
        serviceDTO2.setDstPorts("3");
        ...
        dto.setEndTime("2009-12-9 11:33:22");
        dto.setVsys(true);
        dto.setVsysName("zy");
        SecurityHillStoneR5ForhuaXia hillStoneR5 = new SecurityHillStoneR5ForhuaXia();
