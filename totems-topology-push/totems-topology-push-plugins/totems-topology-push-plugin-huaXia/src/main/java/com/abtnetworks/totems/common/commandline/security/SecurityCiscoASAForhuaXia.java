...
public class SecurityCiscoASAForhuaXia extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = LoggerFactory.getLogger(SecurityCiscoASAForhuaXia.class);
    ...
    public String generateCommandline(CommandlineDTO dto) {
        ...
        SecurityCiscoASAForhuaXia securityCiscoASA = new SecurityCiscoASAForhuaXia();
...
//            dto.setObjectFlag(false);
    ...
    public String commonLine(PolicyObjectDTO srcAddress, PolicyObjectDTO dstAddress, PolicyObjectDTO serviceObject,
        ...
        if (dto.isVsys()){
            if(!AliStringUtils.isEmpty(dto.getVsysName())){
                vsyName = dto.getVsysName();
                sb.append(String.format("changeto context system\n" +
                        "changeto context %s\n",vsyName));
            }
        }
        //定义对象
        ...
        if (createObjFlag || (serviceObject.isObjectFlag() && srcAddress.isObjectFlag() && dstAddress.isObjectFlag())) {
            ...
            String objName1 = String.format("%s_AO_%s", ticket, IdGen.getRandomNumberString());
            dto.setName(objName1);
            String objName2 = String.format("%s_AO_%s", ticket, IdGen.getRandomNumberString());
            dto.setName(objName2);
            sb.append(String.format("access-list %s %s extended %s %s\n\n", businessName, line1, dto.getAction().toLowerCase(), objectString));
            if (dto.getIdleTimeout() != null) {
                sb.append(String.format("class-map %s \nmatch access-list %s\n\n", objName1, businessName));
                ...
                if (minuteTemp > 0) {
                    ...
                    if (hour > 0) {
                        object = (hour >= 10 ? (hour + ""):(hour)) + ":" + (minute >= 10 ? (minute + "") : ("0"+minute))
                                + ":" + (second >= 10 ? (second + "") : ("0"+second));
                    } else {
                        object= "0:" + (minute >= 10 ? (minute + "") : ("0"+minute)) + ":"
                                + (second >= 10 ? (second + "") : ("0"+second));
                    }
                } else {
                    object= "0:0:" + (second >= 10 ? (second + "") : ("0"+second));
                }

                sb.append(String.format("policy-map %s \nclass %s \nset connection timeout idle %s\n\n", objName2, objName1, object));
                if(dto.isOutBound()) {
                    sb.append(String.format("service-policy %s interface %s \n", objName2, dstItfAlias));
                }
                else {
                    sb.append(String.format("service-policy %s interface %s \n", objName2, srcItfAlias));
                }
            }
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, RefObjectDTO refObjectDTO ,int iptype) {
        ...
        if (serviceDTOList.size() == 1 && (singledstport == true||PortUtils.isPortRange(serviceDTOList.get(0).getDstPorts()))) {
            dto.setObjectFlag(false);
            ...
            if (protocolS.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                ...
                if (StringUtils.isNotEmpty(serviceDTOList.get(0).getDstPorts()) && !PolicyConstants.POLICY_STR_VALUE_ANY.equalsIgnoreCase(serviceDTOList.get(0).getDstPorts())&&!PortUtils.isPortRange(serviceDTOList.get(0).getDstPorts()))
                {
                    dto.setJoin(String.format("%s %s %s eq %s", str.toLowerCase(), srcAddressRef, dstAddressRef, serviceDTOList.get(0).getDstPorts()));
                }
                else if(StringUtils.isNotEmpty(serviceDTOList.get(0).getDstPorts()) &&PortUtils.isPortRange(serviceDTOList.get(0).getDstPorts()))
                {
                    String[] split = serviceDTOList.get(0).getDstPorts().split("-");
                    dto.setJoin(String.format("%s %s %s range %s %s", str.toLowerCase(), srcAddressRef, dstAddressRef, split[0],split[1]));
                }
                else {
...
//
////        if(dto.getDstPorts().split(",").length>1){
////            int num=dto.getDstPorts().split(",").length;
////            for(int i=0;i<=num;i++){
...
//    }
    ...
    public static void main(String[] args) {
        ...
        SecurityCiscoASAForhuaXia cisco = new SecurityCiscoASAForhuaXia();
        dto.setVsys(true);
        dto.setVsysName("test");
        dto.setSrcIp("1.3.1.3,43.5.6.7");
        dto.setDstIp("2.3.4.5-4.5.5.5,4.3.5.4,43.7.7.7");
...
//        service.setDstPorts("");
        dto.setIpType(0);
        dto.setIdleTimeout(18001);
        dto.setOutBound(true);
...
//        service.setDstPorts("25,21");
        ...
        service2.setProtocol("6");
//        service1.setDstPorts("323");
//        service2.setDstPorts("");

//        service.setProtocol("6");
        service1.setDstPorts("45-689,46-77");
//        dto.setServiceName("");
        service2.setDstPorts("66,77");
//        ServiceDTO service2 = new ServiceDTO();
