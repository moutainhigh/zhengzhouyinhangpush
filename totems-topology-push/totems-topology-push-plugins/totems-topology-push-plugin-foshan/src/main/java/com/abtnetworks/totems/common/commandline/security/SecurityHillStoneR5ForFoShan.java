...
public class SecurityHillStoneR5ForFoShan extends SecurityPolicyGenerator implements PolicyGenerator {

    private static final Logger logger = Logger.getLogger(SecurityHillStoneR5ForFoShan.class);
    ...
    public SecurityHillStoneR5ForFoShan() {
        init();
    }
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, String ipPrefix, boolean createObjFlag, String existsAddressName, String ipSystem, Integer ipType) {
        ...
        if (arr.length == 1) {
            // 是创建对象
            if (createObjFlag) {
                formatFullAddress(ipAddress, sb, ipPrefix, createObjFlag, dto, ipSystem, ipType);
                sb.append("exit\n");
            ...
            dto.setJoin(String.format("address %s\n", "add_"+Math.abs(num)));
            formatFullAddress(arr,sb,ipType);
            dto.setCommandLine(sb.toString());
            dto.setName(sb.toString());
        }
    ...
    private void formatFullAddress(String address, StringBuilder sb, String ipPrefix, boolean createObjFlag, PolicyObjectDTO dto, String ipSystem, Integer ipType) {
        String name = "";
        String fullStr = "";
        if (ipType.intValue() == IpTypeEnum.IPV4.getCode()) {
            ...
            if (!AliStringUtils.isEmpty(ipSystem)) {
                name  =  String.format("%s_%s", ipSystem, IdGen.getRandomNumberString());
            }
            dto.setName(name);
            sb.append(String.format("address %s\n", name));
            if (dto.getJoin() != null) {
    ...
    public void formatFullAddress(String[] arr, StringBuilder sb, Integer ipType) {
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, Integer idleTimeout, String existsServiceName) {
        ...
        if (groupFlag) {
            ...
            sb.append(String.format("service group %s\n", groupName));
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            String name = String.format("%s%s", protocolString, service.getDstPorts());
            if (idleTimeout != null) {
                name = String.format("%s%s", protocolString, service.getDstPorts());
            }
            ...
            if (idleTimeout != null) {
                sb.append(String.format("service %s_L",name));
            } else {
    ...
    public static void main(String[] args) {
        ...
        serviceDTO.setDstPorts("6817-60,50");
        ...
        SecurityHillStoneR5ForFoShan hillStoneR5 = new SecurityHillStoneR5ForFoShan();
