...
@Slf4j
@Service
@CustomCli(value = DeviceModelNumberEnum.H3CV5, type = PolicyEnum.SECURITY)
public class SecurityH3cSecPathV5ForFoShan extends SecurityPolicyGenerator implements PolicyGenerator {
    ...
    private List<PolicyObjectDTO> generateAddressObject(String ipAddress, String ticket, String existsAddressName) {
        ...
        if (ipAddress.split(",").length == 1) {
            if (IpUtils.isIPRange(ipAddress)) {
                ...
                setName = String.format("object network range %s-%s\n", startIp, endIp);
                ...
                setName = String.format("object network subnet %s_%s\n", ip,maskBit);
            } else {
                setName = String.format("object network host %s\n", ipAddress);
            }
            ...
            setName = String.format("object network %s\n", "add_"+Math.abs(num));
            stringBuilder.append(setName);
        }
        ...
        for (String address : arr) {
            if(IpUtils.isIPRange(address)) {
                ...
                addressCmd = String.format("subnet %s\n", address);
            } else if (IpUtils.isIP(address)){
                addressCmd = String.format("host address %s\n", address);
            }
            stringBuilder.append(addressCmd);
        }
...
//
        dto.setSrcIp("1.1.12.2,2.3.3.4,102.209.1.2/22");
        dto.setDstIp("15.3.2.5");
//        dto.setIpType(1);
        SecurityH3cSecPathV5ForFoShan h3cv5 = new SecurityH3cSecPathV5ForFoShan();
