...
public class SecurityFortinetFortiOSForFoShan extends SecurityPolicyGenerator implements PolicyGenerator {
...
//            sb.append( createAddressObject(dstAddresses,dstAddressName));
        ...
        if (dto.getIdleTimeout() != null) {
            sb.append(String.format("set session-ttl %d\n", dto.getIdleTimeout()));
        }
    ...
    private String createAddressObject(String ip, List<String> addressNameList, CommandlineDTO dto, boolean isSrcIp,int index, int length) {
        ...
        if (dto.getIpType().intValue() == IpTypeEnum.IPV4.getCode()) {
            if (IpUtils.isIPRange(ip)) {
                ...
                sb.append(String.format("edit %s\n", addressObjectName));
                sb.append(String.format("set range %s %s\n", startIp,endIp));
            } else {
                if (IpUtils.isIPSegment(ip)) {
                    ...
                    if(isSrcIp){
                        if(StringUtils.isNotEmpty(dto.getSrcIpSystem())){
                            ...
                            addressObjectName = String.format("\"%s_%s\"", ipFromIpSegment, maskBitFromIpSegment);
                        }
                    } else {
                        if(StringUtils.isNotEmpty(dto.getDstIpSystem())){
                            if(length == 1 ){
                    ...
                    if(isSrcIp){
                        if(StringUtils.isNotEmpty(dto.getSrcIpSystem())){
                            ...
                            addressObjectName = String.format("\"%s\"", ip);
                        }
                        sb.append(String.format("address %s\n", addressObjectName));
                        sb.append(String.format("set ip %s\n", ip));
                    } else {
                        if(IpUtils.isIP(ip)){
                            if(StringUtils.isNotEmpty(dto.getDstIpSystem())){
                                if(length == 1 ){
            ...
            if (IpUtils.isIPv6Range(ip)) {
                String startIp = IpUtils.getRangeStartIPv6(ip);
                String endIp = IpUtils.getRangeEndIPv6(ip);
                if(isSrcIp){
                    if(StringUtils.isNotEmpty(dto.getSrcIpSystem())){
                        if(length == 1 ){
                            addressObjectName = dto.getSrcIpSystem();
                ...
                if (IpUtils.isIPv6Subnet(ip)) {
                    String ipFromIpSegment = IpUtils.getIpSegmentStartIPv6(ip);
                    String maskBitFromIpSegment = IpUtils.getIpSegmentMaskIPv6(ip);
                    if(isSrcIp){
                        if(StringUtils.isNotEmpty(dto.getSrcIpSystem())){
                            if(length == 1 ){
                                addressObjectName = dto.getSrcIpSystem();
    ...
    private String createServiceObject(ServiceDTO service, List<String> serviceNameList) {
        StringBuilder sb = new StringBuilder();
        String protocol = ProtocolUtils.getProtocolByString(service.getProtocol()).toLowerCase();
        ...
        for (String port : dstPortList) {
            ...
            if (!protocol.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                if (PortUtils.isPortRange(port)) {
                    ...
                    serviceName = String.format("\"%s%s-%s\"", protocol, startPort, endPort);
                } else {
                    serviceName = String.format("\"%s%s\"", protocol, port);
                }
            }
            ...
            if (protocol.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_TCP)) {
                if (PortUtils.isPortRange(port)) {
                    String startPort = PortUtils.getStartPort(port);
                    String endPort = PortUtils.getEndPort(port);
                    sb.append(String.format("set tcp-portrange %s_%s\n", startPort, endPort));
                } else {
                ...
                if (PortUtils.isPortRange(port)) {
                    String startPort = PortUtils.getStartPort(port);
                    String endPort = PortUtils.getEndPort(port);
                    sb.append(String.format("set udp-portrange %s_%s\n", startPort, endPort));
                } else {
    ...
    String createPolicy(String theme,String policyName, List<String> srcItfList, List<String> dstItfList,
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("edit %s\n", policyName));
        sb.append("set name ").append(String.format("%s\n", theme));
...
//            }
    ...
    public static void main(String[] args) {
        ...
        SecurityFortinetFortiOS fortinet = new SecurityFortinetFortiOS();
