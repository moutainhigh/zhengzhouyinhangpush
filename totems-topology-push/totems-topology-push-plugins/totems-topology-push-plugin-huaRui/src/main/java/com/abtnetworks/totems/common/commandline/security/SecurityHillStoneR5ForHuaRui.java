...
public class SecurityHillStoneR5ForHuaRui extends SecurityPolicyGenerator implements PolicyGenerator {

    private static final Logger logger = Logger.getLogger(SecurityHillStoneR5ForHuaRui.class);
    ...
    public SecurityHillStoneR5ForHuaRui() {
        init();
    }
    ...
    public String createCommandLine(CommandlineDTO dto) {
        ...
        if(dto.getDescription().contains("-")){
            String[] description=dto.getDescription().split("-");
    ...
    private void formatFullAddress(String address, StringBuilder sb, String ipPrefix, boolean createObjFlag, PolicyObjectDTO dto, String ticket, String ipSystem, int length, Integer ipType,
        ...
        if (ipType.intValue() == IpTypeEnum.IPV4.getCode()) {
            ...
            if (name != null && !name.contains("/") && !name.contains("-")) {
                ...
                String mask = IpUtils.getMaskBitFromIpSegment(name);
    ...
    private PolicyObjectDTO generateTimeObject(String startTime, String endTime, String ticket) {
        ...
        String finstartTimeday=dst.format(startDate).substring(6,8);
        String finendTimeday=dst.format(endDate).substring(6,8);

        String name=finstartTime+"_"+finstartTimemonth+"_"+finstartTimeday+"-"+finendTime+"_"+finendTimemonth+"_"+finendTimeday;
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        if (existServiceNameList.size() > 0) {
            for (String existServiceName : existServiceNameList) {
                if (existServiceName.indexOf("\"") < 0) {
                    existServiceName = "\"" + existServiceName.toUpperCase() + "\"";
                }
                serviceNameList.add(existServiceName.toUpperCase());
            }
        }
        ...
        for (ServiceDTO service : serviceDTOList) {
            ...
            String name = String.format("%s_%s", protocolString, service.getDstPorts());
            if (idleTimeout != null) {
                name = String.format("%s_%s", protocolString, service.getDstPorts());
            }
            if (PortUtils.isPortRange(service.getDstPorts())) {
                ...
                if (idleTimeout != null) {
                    name = String.format("%s%s_%s", protocolString, start, end);
                }
            }
            //name = name.replace("tcp", "T").replace("udp", "UDP");
