 ...
 * @Desc:天融信 Topsec TOS 010-020
 */
@Service
@CustomCli(value = DeviceModelNumberEnum.TOPSEC_TOS_010_020, type = PolicyEnum.SECURITY)
public class SecurityTopsec010020ForGuangZhouNX extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityTopsec010020ForGuangZhouNX.class);
    ...
    public String createCommandLine(CommandlineDTO dto) {
        ...
        //针对广州农信添加日志
        String policyname = String.format("%s", ticket);
        sb.append(String.format("firewall policy add %s log on\n",policyname));
    ...
    public PolicyObjectDTO generateAddressObject(String ipAddress, String ticket, String existsAddressName, String ipSystem, Integer ipType) {
        ...
        for (String address : arr) {
            ...
            if(StringUtils.isNotEmpty(ipSystem)){
                ...
                name = String.format("%s%s", name, DateUtils.getDate().replace("-","").substring(2));
...
//            } else if (IpUtils.isIPSegment(address)) {
                    ...
                    sb.append(String.format("subnet add name %s ipaddr %s mask %s\n", name, ip, IPUtil.getMaskByMaskBit(maskBit)));  //IPUtil.getMaskByMaskBit(maskBit)
                } else {
                    sb.append(String.format("host add name %s ipaddr %s\n", name, address));
                }
            }
    ...
    public PolicyObjectDTO generateTimeObject(String startTime, String endTime, String ticket) {
        ...
        StringBuilder sb = new StringBuilder();
        //starttime格式化  再取后面20200617120000
    ...
    public String getServiceName(ServiceDTO service, Integer ipType, String port) {
        StringBuilder nameSb = new StringBuilder();
        nameSb.append(ProtocolUtils.getProtocolByString(service.getProtocol()));
        int protocolNum = Integer.valueOf(service.getProtocol());
...
//        ipv4
            ...
            if (protocol.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
                ...
                if (port.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY)) {
                    ...
                    nameSb.append("_");
                    System.out.println("这个port：" + port);
                    if (PortUtils.isPortRange(port)) {
                        String start = PortUtils.getStartPort(port);
                        String end = PortUtils.getEndPort(port);
//                        nameSb.append("_");
...
//                        nameSb.append("-");
    ...
    public static void main(String[] args) {
        ...
        SecurityTopsecTos010020ForKunShan topsec = new SecurityTopsecTos010020ForKunShan();
        dto.setIpType(1);
...
//        dto.setEndTime("2008-1-3 10:33:16");
        ...
        SecurityTopsec010020ForGuangZhouNX topsec = new SecurityTopsec010020ForGuangZhouNX();


        List<ServiceDTO> serviceDTOS = new ArrayList<>();
        ServiceDTO serviceDTO = new ServiceDTO();
        serviceDTO.setProtocol("6");
        serviceDTO.setDstPorts("22-90,90");
        serviceDTO.setSrcPorts("33-44,80");
        dto.setServiceList(serviceDTOS);
        serviceDTOS.add(serviceDTO);
        dto.setRestServiceList(serviceDTOS);
       /* dto.setSrcIp("2.12.1.2-12.12.1.10");
        dto.setDstIp("2.2.2.2/10,4.4.4.4");*/
        dto.setSrcIp("a001:3002::4003/127");
        dto.setDstIp("a001:3002::4003-3003:2008::4008");
        dto.setSrcIpSystem("");
        dto.setDstIpSystem("");
        dto.setIpType(1); //0:ipv4 ; 1:ipv6
