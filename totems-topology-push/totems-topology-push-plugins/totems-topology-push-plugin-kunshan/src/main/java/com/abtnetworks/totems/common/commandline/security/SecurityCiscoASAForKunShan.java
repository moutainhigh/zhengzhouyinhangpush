...
public class SecurityCiscoASAForKunShan extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = LoggerFactory.getLogger(SecurityCiscoASAForKunShan.class);
    ...
    public String generateCommandline(CommandlineDTO dto) {
        ...
        PolicyObjectDTO serviceObject = generateServiceObject(dto.getServiceList(), createObjFlag, dto.getServiceName(),ticket);
...
//            String objName = "";
            if (arr.length == 1) {
                String host = "";
                if (IpUtils.isIPRange(ipAddress) || IpUtils.isIPv6Range(ipAddress)) {
                    objName = "N_" + ipAddress;
                    ...
                    host = String.format("subnet %s \n", fullStr);
                } else if (IpUtils.isIPv6Subnet(ipAddress)) {
                    objName = "N_" + ipAddress;
                } else {
                    objName = "H_" + ipAddress;
                    host = "host " +  ipAddress;
                }
                if(StringUtils.isNotEmpty(ipSystem)){
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, boolean createObjFlag, String existsServiceName,String ticket) {
        PolicyObjectDTO dto = new PolicyObjectDTO();
    ...
    public static void main(String[] args) {
        SecurityCiscoASAForKunShan ciscoASA = new SecurityCiscoASAForKunShan();
