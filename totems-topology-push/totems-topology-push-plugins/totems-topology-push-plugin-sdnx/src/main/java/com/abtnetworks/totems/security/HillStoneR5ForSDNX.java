...
public class HillStoneR5ForSDNX extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(HillStoneR5ForSDNX.class);
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        for(ServiceDTO service : serviceDTOList){
            ...
            if(idleTimeout != null) {
                name = String.format("\"%s%sL-long\"", protocolString.substring(0, 1).toUpperCase(), service.getDstPorts());
            }
            if(PortUtils.isPortRange(service.getDstPorts())) {
                ...
                if(idleTimeout != null) {
                    name =  String.format("\"%s%s-%sL-long\"", protocolString.substring(0, 1).toUpperCase(), start, end);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        restSrc.add("2408:8614:0000:0038::E/128");
        restSrc.add("1118:8615:0000:0038::E/128");
        dto.setRestSrcAddressList(restSrc);
        List<String> restDst = new ArrayList<>();
        restDst.add("fd00:1:211:136::2/66");
        restDst.add("fd00:12:222:136::2/66");
        dto.setRestDstAddressList(restDst);
        dto.setIpType(2);
        dto.setIdleTimeout(180);
        HillStoneR5ForSDNX hillStoneR5 = new HillStoneR5ForSDNX();
