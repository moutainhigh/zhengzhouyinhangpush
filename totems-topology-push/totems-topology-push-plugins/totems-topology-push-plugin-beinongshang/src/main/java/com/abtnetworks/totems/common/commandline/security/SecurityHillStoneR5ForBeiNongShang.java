...
public class SecurityHillStoneR5ForBeiNongShang extends SecurityPolicyGenerator implements PolicyGenerator {

    private static Logger logger = Logger.getLogger(SecurityHillStoneR5ForBeiNongShang.class);
    ...
    public SecurityHillStoneR5ForBeiNongShang() {
        init();
    }
    ...
    public String createCommandLine(CommandlineDTO dto) {
        logger.info("HillStoneR5进行策略命令生成开始=======================================================");
        StringBuilder sb = new StringBuilder();
        String ticket = dto.getName();
        ...
        sb.append(String.format("name %s\n", StringUtils.isEmpty(dto.getDescription()) ? dto.getBusinessName() : descripTionCode(dto.getDescription())));
        if(!AliStringUtils.isEmpty(dto.getDescription())){
            sb.append(String.format("description %s\n", descripTionCode(dto.getDescription())));
        }
        ...
        sb.append("exit\n");
        sb.append("end\n");
        logger.info("HillStoneR5进行策略命令生成完毕=======================================================");
        return sb.toString();
    }
    ...
    private void formatFullAddress(String address, StringBuilder sb, String ipPrefix, boolean createObjFlag, PolicyObjectDTO dto, String ticket,String ipSystem,int length, Integer ipType,
        ...
        if(ipType.intValue() == IpTypeEnum.IPV4.getCode()){
            if (IpUtils.isIPSegment(address)) {
                sb.append(String.format("address \"%s\"\n", address));
                addresss =  String.format("\"%s\"\n", address);
                fullStr = String.format("ip %s\n", address);
            } else if (IpUtils.isIPRange(address)) {
                String startIp = IpUtils.getStartIpFromRange(address);
                String endIp = IpUtils.getEndIpFromRange(address);
                sb.append(String.format("address range%s-%s\n", startIp, endIp));
                addresss = String.format("range%s-%s\n", startIp, endIp);
                fullStr = String.format("range %s %s\n", startIp, endIp);
            } else {
                sb.append(String.format("address \"%s/32\"\n", address));
                addresss = String.format("\"%s/32\"\n", address);
                fullStr = String.format("ip %s/32\n", address);
            }
            name = String.format("\"%s\"", name);
            dto.setName(name);
            if (dto.getJoin() != null) {
                dto.setJoin(dto.getJoin() + ipPrefix + "-addr " + addresss);
            } else {
                dto.setJoin(ipPrefix + "-addr " + addresss);
            }
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, Integer idleTimeout, String existsServiceName) {
        ...
        if(groupFlag){
            ...
            logger.info("555555555555555555555555555555555555555" + groupName);
            sb.append(String.format("servgroup %s\n", groupName));
            for(String objName : serviceNameList){
                sb.append(String.format("service %s\n", objName));
            }
            logger.info("666666666666666666666666666666666666666666666666"+sb.toString());
            sb.append("exit\n");
    ...
    public String getServiceNameByNoPort(ServiceDTO service, Integer ipType) {
        ...
        if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY)) {
            ...
            if (StringUtils.isBlank(service.getDstPorts()) || service.getDstPorts().equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ANY)) {
                ...
                command =  protocolString.toUpperCase() + "-ANY ";
            }
        } else if (protocolString.equalsIgnoreCase(PolicyConstants.POLICY_STR_VALUE_ICMP)) {
    ...
    public PolicyObjectDTO generateServiceObject(List<ServiceDTO> serviceDTOList, List<String> existServiceNameList, Integer idleTimeout, Integer ipType) {
        ...
        for(ServiceDTO service : serviceDTOList){
            ...
            if(idleTimeout != null) {
                name = String.format("\"%s%s\"", protocolString.toUpperCase(), service.getDstPorts());
            }
...
//                String[] srcPorts = service.getSrcPorts().split(",");
    ...
    public static void main(String[] args) {
        ...
        commandlineDTO.setName("aaaa");
        commandlineDTO.setSrcIp("10.10.10.1");
        commandlineDTO.setDstIp("102.11.111.0/24");
        commandlineDTO.setDeviceUuid("5bb4529a48474663adcd89609cf4ef9f");
        commandlineDTO.setIpType(0);
        commandlineDTO.setDescription("bbbb");

        commandlineDTO.setMoveSeatEnum(MoveSeatEnum.AFTER);
        ServiceDTO serviceDTO1 = new ServiceDTO();
        serviceDTO1.setProtocol("6");
        serviceDTO1.setDstPorts("0-65535");

        commandlineDTO.setServiceList(Arrays.asList(serviceDTO1));
        commandlineDTO.setAction("PERMIT");
        SecurityHillStoneR5ForBeiNongShang hillStoneR5 = new SecurityHillStoneR5ForBeiNongShang();
        String commandLine = hillStoneR5.composite(commandlineDTO);
