...
import com.abtnetworks.totems.common.commandline.rollback.RollbackSnatFotineForisOS;
...
@Service
@Log4j2
@CustomCli(value = DeviceModelNumberEnum.FORTINET_V5, type = PolicyEnum.SNAT, classPoxy = RollbackSnatFotineForisOS.class)
public class RollbackSnatFotineForisOSV5ForZSZQ implements PolicyGenerator {

    private OverAllGeneratorAbstractBean generatorBean;


    public RollbackSnatFotineForisOSV5ForZSZQ(){
        generatorBean = new NatFortinetForIosImpl();
    }

    @Override
    public String generate(CmdDTO cmdDTO) {
        log.info("开始进入飞塔5.0设备 Snat回滚插件...");
