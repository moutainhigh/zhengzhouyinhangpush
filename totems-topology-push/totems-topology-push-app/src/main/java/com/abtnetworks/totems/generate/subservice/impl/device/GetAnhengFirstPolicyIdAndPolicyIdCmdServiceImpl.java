...
@Slf4j
@Service
public class GetAnhengFirstPolicyIdAndPolicyIdCmdServiceImpl extends GetFirstPolicyIdCmdServiceImpl {
    @Autowired
    ...
    public void modify(CmdDTO cmdDTO) throws Exception {
        ...
        synchronized (GetAnhengFirstPolicyIdAndPolicyIdCmdServiceImpl.class) {
            int currentId = 0;
            if(type == RuleIPTypeEnum.IP4){
                currentId =advancedSettingService.getAnhengPolicyId(deviceUuid);
            }else{
                currentId = advancedSettingService.getAnhengPolicy6Id(deviceUuid);
            }
            if (maxId < currentId) {
                maxId = currentId;
            }
            ...
            if(type == RuleIPTypeEnum.IP4){
                advancedSettingService.setAnhengPolicyId(deviceUuid, newMaxId);
            }else{
                advancedSettingService.setAnhengPolicy6Id(deviceUuid, newMaxId);
            }
