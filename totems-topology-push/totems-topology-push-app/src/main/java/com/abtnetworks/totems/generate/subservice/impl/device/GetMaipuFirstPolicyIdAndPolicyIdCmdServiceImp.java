...
@Slf4j
@Service
public class GetMaipuFirstPolicyIdAndPolicyIdCmdServiceImp extends GetFirstPolicyIdCmdServiceImpl {
    @Autowired
    ...
    public void modify(CmdDTO cmdDTO) throws Exception {
        ...
        synchronized (GetMaipuFirstPolicyIdAndPolicyIdCmdServiceImp.class) {
            int currentId = 0;
            if(type == RuleIPTypeEnum.IP4){
                currentId =advancedSettingService.getMaipuPolicyId(deviceUuid);
            }else{
                currentId = advancedSettingService.getMaipuPolicy6Id(deviceUuid);
            }
            if (maxId < currentId) {
                maxId = currentId;
            }
            ...
            if(type == RuleIPTypeEnum.IP4){
                advancedSettingService.setMaipuPolicyId(deviceUuid, newMaxId);
            }else{
                advancedSettingService.setMaipuPolicy6Id(deviceUuid, newMaxId);
            }
