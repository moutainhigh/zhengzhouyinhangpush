...
public class QuintupleUtils {
    ...
    public static void main(String[] args) throws Exception {
        String existPreIp = "20.20.1.55";
        String checkVO = "20.20.1.0/24";
        String[] sss = ipv46toIpStartEnd(checkVO);

        String[]  existPreIps = existPreIp.split(PolicyConstants.ADDRESS_SEPERATOR);
        String[] targetIps = checkVO.split(PolicyConstants.ADDRESS_SEPERATOR);
        Tmp<String> tem = ipListFilter(Arrays.asList(targetIps),Arrays.asList(existPreIps));
        if(CollectionUtils.isNotEmpty(tem.getPostFilterData())){
            List<String> filterData = tem.getPostFilterData();
            String newPreIP = existPreIp + PolicyConstants.ADDRESS_SEPERATOR + StringUtils.join(filterData, PolicyConstants.ADDRESS_SEPERATOR);
            System.out.println(newPreIP);
     ...
     * @throws UnknownHostException
     */
    public static String[] ipv46toIpList(String ip) throws UnknownHostException {
        String[] startEndStringArray = new String[2];
        ...
        allIpList.add(startEndStringArray[0]);
        Long ipStart = IpUtils.IPv4StringToNum(startEndStringArray[0]);
        Long ipEnd = IpUtils.IPv4StringToNum(startEndStringArray[1]);
        int i = 1;
        while (true) {
            String availableIp = IpUtils.IPv4NumToString(ipStart + i);
            if (!IpUtils.isValidIp(availableIp)) {
                break;
            }
            if (ipEnd > IpUtils.IPv4StringToNum(availableIp)) {
                allIpList.add(availableIp);
            }
            if (ipEnd.equals(IpUtils.IPv4StringToNum(availableIp))) {
                allIpList.add(availableIp);
                break;
            }
            if (ipEnd < IpUtils.IPv4StringToNum(availableIp)) {
        ...
        String[] allIps = allIpList.toArray(new String[allIpList.size()]);
        return allIps;
    }
